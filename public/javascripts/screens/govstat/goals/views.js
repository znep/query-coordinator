(function() {
  ...
  var GoalCard = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      this.listenTo(this.model, 'change:name', function() {
        self.$('h2').text(self.getName());
      });
      this.listenTo(this.model, 'change:category', function() {
        self.updateColor();
      });
      this.listenTo(this.model, 'change:is_public', function(_, isPublic) {
        self.$el.toggleClass('internal', isPublic === false);
      });
      this.listenTo(this.model, 'change:id', function(_, id) {
        self.$('.view.button').attr('href', '/goal/' + id);
      });
    },
    render: function() {
  ...
  var GoalEditor = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      this.listenTo(this.model, 'change:is_public',
        function(_, isPublic) {
          self.$el.toggleClass('draft', !isPublic);
        });

      var titleFields = ['subject', 'comparison_function', 'name', 'goal_delta', 'goal_delta_is_pct', 'end_date'];
      this.listenTo(this.model, _.map(titleFields, function(field) {
          return 'change:' + field;
        }).join(' '),
        function() {
          self.$el.find('.customTitle input').blur();
    ...
    render: function() {
      ...
      // custom components
      // drop in agency list
      var agencyList = new AgencyList({
        collection: this.model.get('agencies'),
        instanceView: AgencyEditor
      });
      agencyList.render();
      ...
      var datasetCardList = new DatasetCardList({
        collection: relatedDatasets,
        instanceView: DatasetCard
      });
      ...
      $relatedDatasets.find('a.addDataset').on('click', function(event) {
        ...
        showDatasetSelect(function(dataset) {
            var datasetProxy = new govstatNS.models.DatasetProxy({
              id: dataset.id
            }, {
              dataset: dataset
            });
      ...
      $dateInputs.each(function() {
        var $this = $(this);
        $this.DatePicker({ // eslint-disable-line new-cap
      ...
      $mainDetails.find('.customTitle input').example(function() {
        return self._generateGoalSentence();
      });
      ...
      $additionalDetails.find('input.titleImageInput').imageUploader({
        ...
        success: function($container, $image, response) {
          self.model.set('title_image', '/api/assets/' + response.id);
        }
      });

      // bind fancy textedit
      var $notes = $additionalDetails.find('.notes');
      $additionalDetails.find('.notesWrapper').on('click', function() {
        $notes.focus();
      });
  ...
  var CategoryPane = Backbone.View.extend({
    ...
    render: function() {
      ...
      var goalsView = new GoalList({
        collection: this.model.goals,
        instanceView: GoalCard
      });
      goalsView.render();
    ...
    saveTitle: function() {
      this.model.set('name', this.$('.categoryTitleEdit').val(), {
        validate: true
      });
      this.uneditTitle();
    },
  ...
  var AgencyList = Backbone.CollectionView.extend({
    ...
    newAgency: function(event) {
      if (event.keyCode < 32) {
        return;
      } // let things progress for special keys
      ...
      this.model.getDataset(function(ds) {
        ...
        if ($.isBlank(ds)) {
          self.$el.append($.tag2({
            _: 'h2',
            className: 'prompt',
            contents: 'Please select a dataset'
          }));
        } else {
  ...
  var ColumnCard = Backbone.View.extend({
    ...
    initialize: function() {
      this.listenTo(this.model.datasetProxy, 'change:id', function() {
        this.updateEntries();
      });
    },
    render: function() {
  ...
  var MetricEditor = Backbone.View.extend({
    ...
    comparisonChanged: function(event) {
      this.model.set('comparison_function', $(event.target).val());
    },
    _addIndicator: function(indicator, childClass) {
      var indicatorEditor = new IndicatorEditor({
        model: indicator
      });
  ...
  var IndicatorEditor = Backbone.View.extend({
    ...
    render: function() {
      ...
      var datasetCard = new DatasetCard({
        model: datasetProxy
      });
      datasetCard.render();
