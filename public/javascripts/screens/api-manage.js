$(function() {
  ...
  init.applications = function() {
    $('.hunum').each(function() {
      $(this).text(blistUtilNS.toHumaneNumber($(this).text())); // eslint-disable-line no-undef
    });

    $('#limitUpdate').click(function() {
      $('.hunum').each(function() {
        $(this).value(blistUtilNS.parseHumaneNumber($(this).value())); // eslint-disable-line no-undef
      });
    });

    $('.limitRemoveButton').click(function() {
  ...
  init.names = function() {
    ...
    function updateColumn(columnFieldName, changes, callback, errorCallback) {
      var column = _.find(blist.dataset.columns, function(col) {
        return col.fieldName === columnFieldName;
      });
      column.update(changes);
      column.save(callback, errorCallback);
    }
  ...
  $('.managesection').each(function() {
    var initFn = init[$(this).attr('id')];
    if (initFn) initFn();
  });
  $('.controlPane').bind('hide', function() {
