...
class Jenkins
  ...
  RELEASE_JOB_NAME = 'frontend-release'
  ...
  def self.find_release_build(release_sha)
    raise "#{RELEASE_JOB_NAME} job not found in jenkins!" unless api.job.exists?(RELEASE_JOB_NAME)
    build_numbers = api.job.get_builds('frontend-release').map {|rel| rel['number']}.sort.reverse
    build_numbers.find do |build_number|
      details = api.job.get_build_details('frontend-release', build_number)
  ...
  def self.get_docker_tag_from_build(build_number)
    ...
    if response.code == '200'
      ...
      raise "Unable to fetch artifact for frontend build #{build_number}"
    end
  end
