          ...
          directive: {
            'li.columnItem': {
              'column<-': {
                ...
                '.columnData@class+': 'columnId#{column.id} #{column.renderTypeName}'
              }
            }
          },
          data: cols,
...
editRRNS.itemDragging = function(ui, primaryPos, linePos) {
  ...
  var addDropSpot = function($item) {
    ...
    lines[lines.length - 1].items.push({
      pos: dropOff[primaryPos],
      beforeItem: $item
    });
  };

  editRRNS.$dropCont.children('.ui-draggable').each(function() {
    var $t = $(this);
...
editRRNS.enableFieldItem = function($item, dragOnly) {
  ...
  if (!dragOnly) {
    ...
    _.defer(function() {
      editRRNS.makeResizable($fieldItem);
    });
  }
...
editRRNS.makeDraggable = function($item, handle,
  ...
  $item.draggable({
    ...
    start: function(e, ui) {
      ...
      var $t = $(this);
      if (setWidth) {
        ui.helper.width($t.width());
      }
      $t.addClass('itemDragging');
      if (_.isFunction(startHandler)) {
        startHandler(e, ui);
      }
    },
    stop: function(e, ui) {
...
editRRNS.makeDroppable = function($item, selector, restrictToChildren, dropped, preDrop) {
  $item.droppable({
    accept: restrictToChildren ? function($draggable) {
    ...
    drop: function(event, ui) {
      ...
      if ($draggedItem.closest('.renderArea').length < 1) {
        $draggedItem = $draggedItem.clone().removeClass('ui-draggable itemDragging');
      }

      if (_.isFunction(preDrop)) {
        preDrop($draggedItem);
      }

      if ($.isBlank(editRRNS.$dropBefore)) {
        $cont.append($draggedItem);
      } else {
        if ($draggedItem.index(editRRNS.$dropBefore) >= 0) {
          return;
        }
        editRRNS.$dropBefore.before($draggedItem);
      }

      if (_.isFunction(dropped)) {
        dropped($draggedItem);
      }
...
editRRNS.addControls = function($item, hasResize) {
  if ($item.children('.controlIndicators').length > 0) {
    return;
  }
  ...
  var $richItem = $item;
  if ($richItem.hasClass('fieldItem')) {
    $richItem = $item.find('.richItem, .richLabel');
  }
  ...
  $item.children('.controlIndicators').find('.settings').menu({
      contents: [{
        ...
        text: 'Reset Width',
        className: 'widthClear',
        href: '#Clear_Width',
        onlyIf: hasResize === true
      }, {
        text: 'Use Absolute Width',
        className: 'widthAbsolute',
        href: '#Absolute_Width',
        onlyIf: hasResize === true
      }, {
        text: 'Use Relative Width',
        className: 'widthRelative',
        href: '#Relative_Width',
        onlyIf: hasResize === true
      }, {
        divider: true
      ...
      onOpen: function($menuC) {
        ...
        $menuC.find('.menuEntry.addColumn').toggleClass('hide', !($item.hasClass('acceptsColumns') || $item.hasClass('renderArea')));
      },
...
editRRNS.makeResizable = function($item, handles) {
  ...
  $item.children('.controlIndicators').prepend($.tag(
    _.map(['nw', 'n', 'ne', 'e', 'se', 's', 'sw', 'w'], function(dir) {
      return {
        tagName: 'span',
        'class': ['resizeIndicator', 'resize' + dir.toUpperCase(), {
          value: 'enabled',
          onlyIf: _.include(handlesArr, dir)
        }]
      };
    }).concat([{
      tagName: 'span',
      'class': ['resizeSize', 'hide']
    }])
  ));
  $item.resizable({
    handles: handles,
    ...
    resize: function(event, ui) {
      ...
      var $ri = $t;
      if ($ri.hasClass('fieldItem')) {
        $ri = $t.find('.richItem, .richLabel');
      }
...
editRRNS.addHoverStates = function($item, selfClass, childContClass) {
  var hoverHandle = function(handle, target) {
    var $handle = $(handle);
    if (editRRNS.inDrag || $.isBlank(target)) {
      $handle.removeClass('directHover hover');
      return;
    }
    ...
    var isSelfResizing = $handle.hasClass('ui-resizable-resizing');
    var isOtherResizing = editRRNS.inResize && !isSelfResizing;
    $handle.
      toggleClass('hover', (handle == target || ($.contains(handle, target) &&
        ($closestSelf.length < 1 || $closestSelf[0] == handle ||
          isSelfResizing))) && !isOtherResizing).
      toggleClass('directHover',
        editRRNS.isDirectTarget(handle, target, childContClass) &&
        !isOtherResizing);
  };

  $item.mouseover(function(e) {
      hoverHandle(this, e.target);
    }).
    mouseout(function(e) {
      hoverHandle(this, e.relatedTarget);
    });
};
...
editRRNS.setUpColumns = function($col) {
  ...
  $col.find('.richColumn').andSelf().each(function() {
    ...
    editRRNS.makeDraggable($c, '> .controlIndicators .columnDrag',
      ...
      function() {
        editRRNS.$renderArea.css('width', 'auto');
      },
      function(e, ui) {
        editRRNS.itemDragging(ui, 'left', 'top');
      });
...
editRRNS.resetConfig = function(previewOnly) {
  var config = ((blist.dataset.metadata || {}).richRendererConfigs || {})[editRRNS.renderType];
...
editRRNS.initLayout = function() {
  ...
  if (editRRNS.renderType == 'richList') {
  ...
  editRRNS.$previewContainer.resizable({
    handles: 'n',
    maxHeight: editRRNS.$container.height() * 0.8,
    minHeight: 30,
    stop: function() {
  ...
  $.live('.renderArea .fieldItem', 'dblclick', function() {
    var $t = $(this).children('.staticLabel');
    if ($t.length < 1 || $t.hasClass('inEdit')) {
      return;
    }

    var t = '';
    if (!$t.hasClass('defaultData')) {
      t = $t.text();
    }
    $t.data('origText', t);
    $t.draggable('disable');
...
(function($) {
  ...
  editRRNS.$dropIndicator = $.tag({
    tagName: 'span',
    'class': 'dropIndicator'
  });
  editRRNS.$dropFinder = $.tag({
    tagName: 'span',
    'class': 'dropFinder',
    contents: '.'
  });
