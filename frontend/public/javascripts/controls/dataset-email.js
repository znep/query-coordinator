$(function() {
  ...
  var awesomeRenderFunction = function(dataItem) {
    return '<p class="title">' + dataItem.displayName + '</p>' +
      '<p class="matchRow"><span class="matchedField">Email:</span> ' + dataItem.email + '</p>';
  };

  var awesomeAchieved = function(dataItem, context) {
    context.closest('div').siblings('.recipientUid').val(dataItem.id).end();
  };
  ...
  blist.dialog.sharing = function(e, owner, dataset) {
      ...
      $eLine.append($.tag({
        ...
        contents: _.map(dataset.getShareTypes(), function(t) {
          return {
            tagName: 'option',
            value: t,
            contents: $.t('core.share_types.' + t.toLowerCase())
          };
        })
      }));
    ...
    if (!dataset.isPublic()) {
      $flash.addClass('notice').text($.t('screens.ds.email.private_notice', {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          friends = _.map(_.select(data, function(u) {
            ...
            return {
              displayName: $.htmlEscape(friend.displayName),
              email: friend.email,
              id: friend.id
            };
          });
  ...
  var doSetup = function() {
    ...
    $('.emailDatasetDialog .addMoreRecipientsButton').click(function(e) {
      ...
      var $emailClone = $form.find('.emailLine').first().clone().
        find('.autocomplete').remove().end().
        find('label.error').remove().end().
        find('input').val('').end().
        find('.removeLink').removeClass('hiddenLink').end();

      var $select = $emailClone.find('.recipientRole');
      // Non-admins won't have sharing dropdown
      if ($select.length > 0) {
        $select.get(0).selectedIndex = 0;
        $emailClone.find('.selector.uniform').replaceWith($select);
        $select.uniform();
      }

      $emailClone.insertAfter($form.find('.emailLine:last'));

      var $emailField = $emailClone.find('.emailRecipient').attr('name', 'emailRecipient' + emailCount);
      ...
      if ($form.valid()) {
        ...
        ServerModel.sendBatch(refreshCallback,
          function(errorMessage) {
            $flash.addClass('error').text(
              errorMessage || 'There was an error sending your email. Please try again later.'
            );
          });
      }
    });
    ...
    $('.emailDatasetContent .cancel').click(function() {
