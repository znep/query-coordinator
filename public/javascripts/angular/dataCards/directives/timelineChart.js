var templateUrl = require('angular_templates/dataCards/timelineChart.html');
  ...
  return {
    ...
    link: function(scope, element) {
        ...
        switch (labelPrecisionToUse) {
          ...
          case 'MONTH':
            if (useFullMonthNames) {
              label = `${moment(labelDate).format('MMMM')} ${labelDate.getFullYear()}`;
            } else {
              label = `${moment(labelDate).format('MMM')} '${`0${labelDate.getFullYear() % 100}`.slice(-2)}`;
            }
            break;

          case 'DAY':
            if (useFullMonthNames) {
              label = `${labelDate.getDate()} ${moment(labelDate).format('MMMM')} ${labelDate.getFullYear()}`;
            } else {
              label = `${labelDate.getDate()} ${moment(labelDate).format('MMM')}`;
            }
            break;

          default:
            throw new Error(
              `Cannot format date label for unrecognized unit "${labelPrecisionToUse}".`);
      ...
      function formatDateRangeLabel(startDate, endDate) {
        ...
        return `${label} <span class="timeline-chart-clear-selection-button">×</span>`;
      ...
      function renderChartSelection() {
        ...
        if (!_.isNull(minDate) && !_.isNull(maxDate)) {
          ...
          dateRangeFlyoutLabel = `${formatDateLabel(minDate, true)} - ${formatDateLabel(maxDate, true)}`;
      ...
      function renderFlyout(target) {
        ...
        var formatFlyoutValue = function(unit, value) {
          ...
          formattedValue = (_.isFinite(value)) ?
            `${$window.socrata.utils.formatNumber(value)} ${unit}` :
            I18n.common.noValue;
      ...
      function renderSelectionMarkerFlyout() {
        if (!currentlyDragging) {
          return `<div class="flyout-title">${I18n.timelineChart.dragHelp}</div>`;
        }
      }

      function renderClearSelectionMarkerFlyout() {
        if (mousePositionWithinChartLabels) {
          return `<div class="flyout-title">${I18n.timelineChart.dragClearHelp}</div>`;
        }
      }
