module SocrataSiteChrome
  class Configuration < Model
    ...
    def self.find_unmerged(id)
      path = "/#{name.pluralize.downcase}/#{id}.json?merge=false"
      parse(CoreServer::Base.connection.get_request(path))
    end

    def self.get_or_create(type, opts)
      config = find_by_type(type, true, SocrataSiteChrome::CurrentDomain.cname).first
      if config.nil?
        config = create({'type' => type, 'default' => true, 'domainCName' => SocrataSiteChrome::CurrentDomain.cname}.merge(opts))
      end
      config
    end
    ...
    def strings(locale = nil)
      ...
      locale = I18n.locale unless locale.present? || I18n.locale.to_s == SocrataSiteChrome::CurrentDomain.default_locale
    ...
    def create_property(name, value, batch_id = nil)
      ...
      CoreServer::Base.connection.create_request(
        url, { 'name' => name, 'value' => value }.to_json, {}, false, batch_id
      )
    end
    ...
    def update_property(name, value, batch_id = nil)
      ...
      CoreServer::Base.connection.update_request(
        url, { 'name' => name, 'value' => value }.to_json, {}, batch_id
      )
    end
