(function($) {
  ...
  var animateSlices = function($storiesContainer, storiesContainerWidth, $lastImage, $nextImage, $slices) {
    ...
    $slices.css('background-color', $nextImage.css('background-color')).
      ...
      each(function(i) {
        ...
        $slice.css('left', targetPosition).css('background-position', targetBgOffset + 'px 0');

        $slice.css('top', 100 * ((i % 2 === 0) ? -1 : 1));

        setTimeout(function() {
          $slice.show().animate({ top: 0 }, 400).animate({ opacity: 1 }, { duration: 700, complete: function() {
  ...
  $(function() {
    ...
    _.times(30, function() {
      $storiesContainer.append('<div class="slice"></div>');
    });
    var $slices = $storiesContainer.children('.slice');
    $slices.css('z-index', 1).css('position', 'absolute').css('background-repeat', 'no-repeat').hide();
