    ...
    return $.each(this, function() {
      ...
      var tip = $this.socrataTip({
        ...
        shownCallback: function(box) {
          $(box).find('.popupSelectList').lockScroll(true);
        },
        stroke: '#444444',
        trigger: 'click'
      });
      ...
      if (!eventsBound) {
        $.live('.popupSelectList li', 'click', function() {
          var $item = $(this);

          var data = popups[$item.closest('.popupSelectContainer').attr('data-popupid')];

          if (data.opts.selectCallback(items[$item.attr('data-itemid')], !$item.hasClass('checked'))) {
