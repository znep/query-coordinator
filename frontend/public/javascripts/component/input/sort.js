(function($) {
    ...
    _initDom: function() {
      ...
      if ($.isBlank(cObj.$dsListDropdown)) {
        ...
        if (cObj.$dsListDropdown.length < 1) {
          cObj.$dsListDropdown = $.tag2({
            _: 'select',
            id: cObj.id + '-dsListSort',
            name: 'dsListSort',
            className: 'dsListSort'
          });
        ...
        cObj.$dsListDropdown.on('change', function() {
          _.defer(function() {
            handleChange(cObj);
          });
        });
      ...
      if ($.isBlank(cObj.$dsListPeriodDropdown)) {
        ...
        if (cObj.$dsListPeriodDropdown.length < 1) {
          cObj.$dsListPeriodDropdown = $.tag2({
            _: 'select',
            id: cObj.id + '-dsListSortPeriod',
            name: 'dsListSortPeriod',
            className: 'sortPeriod'
          });
        ...
        cObj.$dsListPeriodDropdown.on('change', function() {
          _.defer(function() {
            handleChange(cObj);
          });
        });
      ...
      if ($.isBlank(this.$dsDropdown)) {
        ...
        this.$dsDropdown.on('change', function() {
          _.defer(function() {
            handleChange(cObj);
          });
        });
  ...
  var renderUpdate = function() {
    ...
    if (cObj._dataContext.type == 'dataset') {
      ...
      if (cObj._curDataset != ds) {
        ...
        cObj._curDataset = ds;
        cObj._curDataset.bind('query_change', function() {
          renderUpdate.apply(cObj);
        });
        cObj._curDataset.bind('columns_changed', function() {
          renderUpdate.apply(cObj);
        });
      }
      ...
      var curSort = _.first(ds.query.orderBys || []) || {
        expression: {}
      };
      var foundCur = false;
      var selVal = '';
      ...
      cObj.$sortDir.
        removeClass('hide').
        toggleClass('sortAsc', col.sortAscending === true).
        ...
        cObj.$sortClear.
          toggleClass('hide', $.isBlank(col.sortAscending)).
      ...
      if (cObj.$dsListDropdown.children().length < 1) {
        cObj.$dsListDropdown.append($.tag2(
          _.map(['relevance', 'most_accessed', 'alpha',
            'newest', 'oldest', 'last_modified', 'rating', 'comments'
          ], function(opt) {
            return {
              _: 'option',
              value: opt,
              contents: $.t('controls.browse.sorts.' + opt)
            };
          })));
      }
  ...
  var handleChange = function(cObj) {
    if ($.subKeyDefined(cObj, '_dataContext.dataset')) {
      ...
      cObj.$sortDir.
        toggleClass('hide', $.isBlank(cfn)).
        removeClass('sortDesc sortAscLight').
        addClass('sortAsc');
    } else if (cObj._dataContext.type == 'datasetList') {
