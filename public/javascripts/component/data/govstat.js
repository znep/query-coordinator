...
$.component.Component.extend('GovStat', 'none', {
    ...
    _render: function() {
        ...
        if (!cObj._updateDataSource(cObj._properties, cObj._gsRender)) { cObj._gsRender(); }
    },

    _gsRender: function() { /* implement me */ },
    ...
    _gsLoad: function(objReq, opts) {
        ...
        objReq.map(function(result) {
            if (result instanceof blist.require('janus').store.Request.state.type.Success) {
                ...
                if (!$.isBlank(view)) { cObj.$contents.append(view.artifact()); }
    ...
    _propWrite: function(properties) {
        ...
        if (!_.isEmpty(properties)) { cObj._render(); }
    }
});
...
$.component.GovStat.extend('GovStat Goal', 'none', {
    _gsRender: function() {
        ...
        if (context != 'card' && context != 'detail') { context = 'card'; }
