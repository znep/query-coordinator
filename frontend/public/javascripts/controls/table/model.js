...
(function($) {
  ...
  blist.data.Model = function() {
            ...
            bind('row_change', function(rows, fullReset) {
              _.each($.makeArray(rows), function(r) {
                if (fullReset) {
                  ...
                  self.view.trigger('row_change', [r.childRows]);
                }
              });
            }, this);
    ...
    this.saveRowValue = function(value, row, column, isValid, skipUndo) {
      ...
      if (!$.isBlank(column)) {
        if ($.isBlank(childRow)) {
          ...
          this.view.setRowValue(value, childRow.id, column.id, !isValid, parRow.id, parCol.id);
      ...
      switch (item.type) {
        case 'edit':
          ...
          if ($.isBlank(curValue)) {
            ...
            valueIsInvalid = !$.isBlank(curValue);
          }
          oppItem = {
            type: 'edit',
            value: curValue,
    ...
    this.index = function(row) {
      ...
      if (!$.isBlank(specialLookup[row.id])) {
        return row.index;
      }
      return row.index + countSpecialTo(row.index + 1) - this.currentBucketStart();
    };
    ...
    this.get = function(index) {
      ...
      return specialRows[index + this.currentBucketStart()] ||
        this.view.rowForIndex(index - countSpecialTo(index, true) + this.currentBucketStart());
    };
    ...
    this.length = function(accountForBuckets) {
      ...
      if (accountForBuckets && this.usingBuckets() && !_.isNull(this.currentBucket)) {
        return Math.min(
          ...
          this.dataLength() - this.currentBucketStart() + specialCount
        );
      } else {
    ...
    this.selectRowsTo = function(row) {
      ...
      var minIndex = _.min(_.map(this.view.highlightTypes.select, function(selected) {
    ...
    var getChildRows = function(row) {
      ...
      for (var i = 0; i < cols.length; i++) {
        ...
        if ($.isBlank(cell) && !self.useBlankRows()) {
          continue;
        }
        cell = cell || [];
    ...
    this.expand = function(row, open, skipEvent) {
      ...
      if (open) {
        ...
        _.each(childRows, function(cr) {
          specialLookup[cr.id] = cr;
        });
    ...
    this.currentBucketStart = function() {
      if (_.isNull(this.currentBucket)) {
        return 0;
      }

      return this.currentBucket.start;
    };

    this.currentBucketIndexOrUndefined = function() {
      if (_.isNull(this.currentBucket)) {
        return;
      }
    ...
    var countSpecialTo = function(max, isAbsolute) {
      ...
      if (max > specialCount) {
        _.each(specialRows, function(r, index) {
          if (index < max) {
            count++;
          }
        });
    ...
    var doExpansion = function() {
      ...
      _.each(missingRows, function(rId) {
        delete expandedRows[rId];
      });
  ...
  $.fn.extend({
    ...
    blistModel: function(model) {
      ...
      var thisModel = this.data('blistModel');
      if (thisModel) {
        return thisModel;
      }
