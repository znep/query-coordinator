...
module.exports = function ApiExplorer($window, http, WindowState, rx) {
  ...
  return {
    ...
    link: function($scope, element) {
      ...
      // parameters are set: $$read_from_nbe=true&$$version=2.1. Since we don't want to publicly
      // expose these flags to consumers, we're hiding the geojson endpoint completely for derived
      // views.
      var geoJsonAvailable$ = $scope.dataset$.
        pluck('isFromDerivedView').
        flatMap(function(isFromDerivedView) {
          if (_.isEqual(isFromDerivedView, true)) {
            return Rx.Observable.returnValue(false);
          } else {
            return datasetId$.
