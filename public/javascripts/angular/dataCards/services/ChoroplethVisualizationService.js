...
function ChoroplethVisualizationService(Constants, $window) {
  ...
  ChoroplethVisualizationUtils.prototype.calculateColoringScale = function(colorRange, classBreaks) {
    ...
    if (this.qualitativeColors === colorRange) {
      if (classBreaks.length > colorRange.length) {
        throw new Error(`Cannot calculate qualitative coloring parameters for more than ${this.qualitativeColors.length} class breaks.`);
      }
  ...
  ChoroplethVisualizationUtils.prototype.fillColor = function(colorScale, feature) {
    ...
    var unfilteredValue = _.get(feature, `properties.${Constants.UNFILTERED_VALUE_PROPERTY_NAME}`);
    var filteredValue = _.get(feature, `properties.${Constants.FILTERED_VALUE_PROPERTY_NAME}`);
  ...
  ChoroplethVisualizationUtils.prototype.strokeColor = function(colorScale, feature, highlighted) {
    ...
    if (!_.hasValue(_.get(feature, `properties.${Constants.FILTERED_VALUE_PROPERTY_NAME}`, undefined)) ||
  ...
  ChoroplethVisualizationUtils.prototype.getStyleFn = function(colorScale) {
    ...
    return function(feature) {
      ...
      if (_.hasValue(_.get(feature, `properties.${selectedPropertyName}`, undefined))) {
