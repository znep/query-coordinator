(function(root) {
  ...
  function ColumnChart(element, vif) {
    ...
    function _renderTemplate(thisElement) {
      ...
      thisElement.append(chartContainer);
    }

    function _attachEvents(thisElement) {
    ...
    function _detachEvents(thisElement) {
    ...
    function showFlyout(event) {
      ...
      var barGroupElement = _chartWrapper.
        find('.bar-group').
        filter(function(index, thisElement) { return thisElement.getAttribute('data-bar-name') === barGroupName; }).
        find('.unfiltered').
        get(0);
    ...
    function addHoverClassToBarGroup(event) {
      var datum = d3.select(event.currentTarget).datum();
      var barName = _escapeQuotesAndBackslashes(datum[NAME_INDEX]);

      _chartWrapper.
        find('.bar-group').
        filter(function(index, thisElement) { return thisElement.getAttribute('data-bar-name') === barName; }).
        addClass('highlight');
    }
    ...
    function _renderData(thisElement, data, options) {

      // Cache dimensions and options
      var chartWidth = thisElement.width();
      var chartHeight = thisElement.height();
      ...
      var calculatedChartHeight = Math.max(0, chartHeight - topMargin - bottomMargin - horizontalScrollbarHeight);
      ...
      var verticalScale = _computeVerticalScale(calculatedChartHeight, chartDataRelevantForVerticalScale, showFiltered);

      var chartLeftOffset = horizontalScale.range()[0];
      var chartRightEdge = chartWidth - chartLeftOffset;

      _chartWrapper.css('height', calculatedChartHeight + topMargin + 1);
      _chartScroll.css({
        'padding-top': 0,
        'padding-bottom': bottomMargin,
        'top': 'initial',
        'width': chartWidth,
        'height': calculatedChartHeight + topMargin + horizontalScrollbarHeight
      });
      ...
      var _renderTicks = function() {
        ...
        var tickMarks = _.uniq([0].concat(verticalScale.ticks(numberOfTicks))).map(function(tickValue) {
          ...
          var tickTopOffset = calculatedChartHeight - verticalScale(tickValue);
        ...
        return $('<div>', {
          ...
          css: {
            ...
            width: chartWidth,
            height: calculatedChartHeight + topMargin
          }
        }).append(tickMarks);
      };

      var updateLabels = function(thisLabelSelection) {
        ...
        // The `_.property(NAME_INDEX)` below is equivalent to `function(d) { return d[NAME_INDEX]; }`
        var labelDivSelection = thisLabelSelection.data(labelData, _.property(NAME_INDEX));
        ...
        labelDivSelection.
          ...
          each(function(d) {
            ...
            var newLabelSelection = d3.select(this);
            var labelContentSelection = newLabelSelection.select('.contents');
            ...
            // Apply styles
            newLabelSelection.style('left', '{0}px'.format(labelLeftOffset));
            newLabelSelection.style('right', '{0}px'.format(labelRightOffset));
