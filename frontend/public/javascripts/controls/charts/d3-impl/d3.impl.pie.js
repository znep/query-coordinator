  ...
  $.Control.registerMixin('d3_impl_pie', {
    ...
    initializeVisualization: function() {
      ...
      $dom.empty().append($.tag({
        tagName: 'div',
        'class': 'mondrian pieChart orientation' + $.capitalize(cc.orientation),
        contents: [{
          tagName: 'div',
    ...
    _loadDataIfNeeded: function() {
      ...
      if (vizObj._isSortedBigToSmall()) {
        ...
        topDone = vizObj._normalizeAngle(zoom.getRightmostAngle()) > vizObj._normalizeAngle(topAngle);
        bottomDone = vizObj._normalizeAngle(zoom.getLeftmostAngle()) < vizObj._normalizeAngle(bottomAngle);
        ...
        topDone = (state.top == minIndex);
        bottomDone = (state.bottom == staticMaxIndex);
    ...
    renderData: function(data, view, addedNewData) {
      this._renderData(data, addedNewData);
    },
    ...
    getRenderRange: function() {
      ...
      var ret = {
        start: state.top,
        length: 1 + (state.bottom - state.top)
      };
    ...
    _renderUnusableData: function(data, reason) {
      ...
      var valueResolver = $.thunk(0);
      var getDataSum = $.thunk(0);
      ...
      var updateSlices = slices.
        ...
        on('click', function(datum) {
          ...
          if (datum) {
            ...
            if (!row) {
              return;
            } // We might not have a row if this is 'other'.
    ...
    _renderLabels: function(pieces, seriesInformation, arc, sliceRadius, fillArea, translateX, translateY, idFunction) {
      ...
      var filterToShowable = function(toFilter) {
        ...
        return {
          unshowableLabelsWithSpace: unshowableLabelsWithSpace,
          showable: filtered
        };
      };
      ...
      labels.
        ...
        attr('y', function(d) {
          return d.slottedCircleLayout.y;
        }).
        text(function(d) {
          return d.fittedText;
        });
      ...
      lines.
        attr('d', function(d, i) {
          ...
          if (leftOfCenter) {
            ...
            // Target point is below us and to the right (quadrant IV).
            if (pointJustOutsideSlice[1] > pointOnLabel[1] && pointJustOutsideSlice[0] > pointOnLabel[0]) {
              ...
              // Other quadrants pretty much get a straight line with simple elbows.
          ...
          if (teeVertical) {
            path += vizObj._d3_line_path(
              [pointOnLabel[0], pointOnLabel[1] - teeLengthSide], [pointOnLabel[0], pointOnLabel[1] + teeLengthSide]);
          } else {
            path += vizObj._d3_line_path(
              [pointOnLabel[0] - teeLengthSide, pointOnLabel[1]], [pointOnLabel[0] + teeLengthSide, pointOnLabel[1]]);
          }
    ...
    _calculateSliceSetMetrics: function(slices, seriesInformation) {
      var sliceSum = _.reduce(slices, function(memo, slice) {
        return memo + slice.getValue();
      }, 0);
