...
to load only the DSLP (Data Set Landing Page) bundle run:
    ...
    FRONTEND_WEBPACK_BUNDLES=admin,dataset-landing-page npm run webpack-dev-server

To load only the CLP (Catalog Landing Page) bundle run:

```
FRONTEND_WEBPACK_BUNDLES=catalog-landing-page bundle exec foreman start
```

If you're curious about the list of webpack bundles that exist and that you might be interested in using, look
in the `config/webpack/` directory. Each `.config.js` file corresponds to a loadable bundle.
