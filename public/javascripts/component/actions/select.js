...
$.component.Component.extend('Select', 'actions', {
  ...
  _initDom: function() {
    ...
    if ($.isBlank(cObj.$select)) {
      ...
      if ($.isPresent($.uniform)) {
        cObj.$select.uniform();
      }

      cObj.$select.off('.compSelect');
      cObj.$select.on('change.compSelect', function() {
        // TODO Support multiple selection
        var selectedOption = cObj.$select.find('option:selected');

        cObj.trigger('change', [{
          dataContext: cObj._dataContext,
          selected: {
...
var doRender = function(cObj) {
  ...
  if (_.isEmpty(options) && $.isPresent(cObj._properties.rowFields) && $.subKeyDefined(cObj, '_dataContext.dataset')) {
