...
- `config` contains [webpack](https://webpack.github.io/docs/configuration.html) configuration
  ...
  ultimately passed to webpack. There is also a file `config/webpack/shared.config.js`,
  which contains a shared webpack configuration for each JavaScript application.
  Note: there are currently two exceptions that have their own webpack configurations:
  `config/webpack/data-cards.config.js` and `config/webpack/open-data.config.js`.
...
Note: in January 2018 the majority of the various webpack bundles have been migrated into
a single shared config (`config/webpack/shared.config.js`). There are currently two configs
that are not migrated: `open-data.config.js` and `data-cards.config.js`. They are older and
difficult to bring into the shared configuration. All new apps should be put in the shared config.
All other apps are included in the shared config at the `entry` key in `config/webpack/shared.config.js`
