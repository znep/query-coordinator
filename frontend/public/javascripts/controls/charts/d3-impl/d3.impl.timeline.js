(function($) {
  ...
  $.Control.registerMixin('d3_impl_timeline', {
    ...
    _interpolateMissingDates: function(data) {
      ...
      var moments = _.map(allDates, function(date) {
          return moment(date);
        }).sort(sortFunc),
        durations = _.chain(_.range(0, allDates.length - 1, 1)).
        map(function(i) {
          return moments[i + 1].diff(moments[i]);
        }).
      ...
      if (inaccuracyIndex > 0.60) {
        ...
        var mostLikelyIncrement = _.max(durations, function(ary) {
          return ary.length;
        })[0];
    ...
    _numVisibleRows: function() {
      if (!this._chartConfig.dateScale) {
        return;
      }
