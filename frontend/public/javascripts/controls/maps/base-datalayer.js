(function($) {
  $.Control.extend('socrataDataLayer', {
    ...
    bindDatasetEvents: function() {
      ...
      if (layerObj._eventsBound) {
        return;
      }

      layerObj._eventsBound = {
        'row_change': function(rows, fr) {
          layerObj.handleRowChange(rows, fr);
        },
        'displayformat_change': function(df) {
          layerObj.handleDisplayFormatChange(df);
        },
        'query_change': function() {
          layerObj.handleQueryChange();
        },
        'conditionalformatting_change': function() {
          layerObj._parent._controls.Overview.redraw();
        },
        'set_temporary': function() {
          layerObj.fireTemporaryEvent(true);
        },
        'clear_temporary': function() {
          layerObj.fireTemporaryEvent(false);
        }
      };
    ...
    handleDisplayFormatChange: function(newDF) {
      ...
      var changes = [];
      _.each(layerObj.discoverDisplayFormatChanges(), function(def) {
        if (_.any(def.keys, comparator)) {
          changes.push({
            callback: def.onChange,
            args: def.args,
      ...
      _.each(changes, function(c) {
        c.callback.apply(c.scope, [c.args]);
      });
    },
    ...
    discoverDisplayFormatChanges: function() {
      return {
        highlight: {
          keys: ['highlightColor'],
          onChange: function() {
            this._highlightColor = this._displayFormat.highlightColor;
          }
        },
        opacity: {
          keys: ['opacity'],
          onChange: this.reloadOpacity
        },
        flyouts: {
          keys: ['flyoutsNoLabel', 'plot.titleId', 'plot.descriptionColumns'],
    ...
    handleRowChange: function(rows, fullReset) {
      ...
      // This is because row_change(fullReset: true) appears
    ...
    initializeColumns: function() {
      ...
      layerObj._geoCol = _.detect(view.realColumns, function(col) {
        return _.include(['geospatial', 'location', 'point'], col.renderTypeName);
      });
      ...
      if (!$.isBlank(layerObj._displayFormat.plot.locationId)) {
        layerObj._locCol = view.columnForIdentifier(layerObj._displayFormat.plot.locationId);
      }

      layerObj._redirectCol = view.columnForIdentifier(layerObj._displayFormat.plot.redirectId);

      layerObj._iconCol = view.columnForIdentifier(layerObj._displayFormat.plot.iconId);

      var aggs = {};
      _.each(['colorValue', 'sizeValue', 'quantity'], function(colName) {
        var c = view.columnForIdentifier(layerObj._displayFormat.plot[colName + 'Id']);
        if (!$.isBlank(c)) {
      ...
      if (!_.isEmpty(aggs)) {
        var gradient = _.map(
          $.gradient(layerObj.settings.numSegments, layerObj._displayFormat.color || '#0000ff'),
          function(c) {
            return '#' + $.rgbToHex(c);
          }
        );
        ...
        view.getAggregates(function() {
          _.each(aggs, function(a, cId) {
            ...
            if (!layerObj._segments) {
              layerObj._segments = {};
            }

            layerObj._segments[column.id] = [];

            for (var i = 0; i < layerObj.settings.numSegments; i++) {
            ...
            if (layerObj._colorValueCol == column) {
              layerObj._colorSpread = {
                min: column.aggregates.minimum,
    ...
    initializeFlyouts: function() {
      ...
      if (!layerObj._$flyoutTemplate) {
        layerObj._$flyoutTemplate = layerObj._parent.$dom().siblings(
          '.flyoutRenderer.template[data-layerId="' + layerObj._uniqueId + '"]'
        );

        if (layerObj._$flyoutTemplate.length < 1) {
          layerObj.$dom().after($.tag({
            tagName: 'div',
            'data-layerId': layerObj._uniqueId,
            'class': ['template', 'row', 'richRendererContainer', 'flyoutRenderer']
          }));

          layerObj._$flyoutTemplate = layerObj.$dom().siblings(
            '.flyoutRenderer.template[data-layerId="' + layerObj._uniqueId + '"]'
          );
        }
    ...
    getFlyout: function(rows) {
      ...
      _.each(rows, function(r) {
        $info.append(layerObj.renderFlyout(r));
      });
    ...
    addInfoPagingToFlyout: function($flyout) {
      ...
      $flyout.append($.tag({
        ...
        contents: [{
          ...
          href: '#Previous',
          title: $.t('controls.map.previous_row'),
          contents: '&lt; ' + $.t('controls.map.previous')
        }, {
          tagName: 'a',
          'class': 'next',
          href: '#Next',
          title: $.t('controls.map.next_row'),
          contents: $.t('controls.map.next') + ' &gt;'
        }]
      }));
    },
    ...
    setQuery: function(query) {
      ...
      if ($.isBlank(queryClone.namedFilters)) {
        queryClone.namedFilters = {};
      }
  ...
  $.Control.registerMixin('points', {
      ...
      discoverDisplayFormatChanges: function() {
        ...
        return $.extend(true, layerObj._super(), {
          rowreload: {
            keys: layerObj.displayFormatKeyChanges(),
            onChange: function() {
              layerObj.handleDataLoaded(layerObj._view.loadedRows());
              layerObj._parent._controls.Overview.redraw();
            }
      ...
      highlightRows: function() {
        var view = this._view,
          args = arguments;
        _.defer(function() {
          view.highlightRows.apply(view, args);
        });
      },
      ...
      unhighlightRows: function() {
        var view = this._view,
          args = arguments;
        _.defer(function() {
          view.unhighlightRows.apply(view, args);
        });
      },
          ...
          layerObj.flyoutHandler().add(
            ...
            feature.attributes.flyout[0].innerHTML, {
              ...
              atPixel: feature.geometry instanceof OpenLayers.Geometry.Polygon ?
                layerObj._parent._lastClickAt.clone() : false
            }
          );
        }
      },
      ...
      prepareRowRender: function(row) {
        ...
        var data = {
          icon: this.extractIconFromRow(row)
        };
        if (!data.icon) {
          data = {
            color: this.extractColorFromRow(row),
      ...
      extractGeometryFromRow: function(row) {
        ...
        if (!$.isBlank(locCol)) {
          ...
          lonlat = new OpenLayers.LonLat(
            parseFloat(row.data[layerObj._longCol.lookup]),
            parseFloat(row.data[layerObj._latCol.lookup])
          );
        }

        // Incomplete points will be safely ignored
        if (lonlat && lonlat.isIncomplete()) {
          return 'lonlat is incomplete';
        }
      ...
      extractColorFromRow: function(row) {
        ...
        if ($.subKeyDefined(row, 'metadata.meta.pinColor')) {
          ...
          var matchingSegment = _.detect(layerObj._segments[layerObj._colorValueCol.id], function(segment) {
            return parseFloat(row.data[layerObj._colorValueCol.lookup]) <= segment.value;
          });

          if (matchingSegment) {
            return matchingSegment.color;
          }
        }
      ...
      extractSizeFromRow: function(row) {
        ...
        if ($.subKeyDefined(row, 'metadata.meta.pinSize')) {
          ...
          var matchingSegment = _.detect(layerObj._segments[layerObj._sizeValueCol.id], function(segment) {
            return parseFloat(row.data[layerObj._sizeValueCol.lookup]) <= segment.value;
          });

          if (matchingSegment) {
            return matchingSegment.size;
          }
        }
        return 0;
      }
