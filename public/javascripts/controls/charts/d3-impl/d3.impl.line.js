(function($) {
  ...
  $.Control.registerMixin('d3_impl_line', {
    ...
    _computeClippingRectForColumnAndScale: function(col, scale) {
      ...
      _.each(valueColumns, function(colDef, seriesIndex) {
        ...
        if (vizObj._seriesGrouping) {
          ...
          var sortIndex = vizObj._chartType == 'timeline' ?
            ...
            function(d) {
              return d.data[cc.fixedColumn.lookup];
            } : 'index';
        ...
        if (vizObj._displayFormat.dataLabels === true) {
          ...
          dataLabels.
            ...
            attr('y', function(d, i) {
              ...
              var tangent = 0;
              if (before) {
                tangent += before < datum ? 1 : -1;
              }
              if (after) {
                tangent -= after < datum ? 1 : -1;
              }
              if (axisDelta >= -0.5 && axisDelta < 15) {
                tangent = -1;
              } else if (axisDelta < -0.5 && axisDelta > -15) {
                tangent = 1;
              }
    ...
    _rerenderAxis: function() {
      ...
      var oldYPos = vizObj._yDatumPosition(function() {
      ...
      _.each(valueColumns, function(colDef, seriesIndex) {
        ...
        if (!_.isUndefined(yScale)) {
          dataBars.
            transition().
            duration(vizObj._animationLengthMillisec).
            attr('cy', vizObj._yDatumPosition(function() {
              return this.__dataColumn.lookup;
            }, yScale)).
            each('end', _.once(function() {
              yScale.commit();
