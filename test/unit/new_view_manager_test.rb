...
class NewViewManagerTest < Test::Unit::TestCase
  ...
  def test_create_creates_a_new_data_lens
    ...
    created = false
    published = false
    connection_stub.expects(:create_request).times(2).then.with do |url, payload|
      if '/views.json?accessType=WEBSITE' == url
        created = true
      ...
      elsif '/views/asdf-asdf/publication.json?accessType=WEBSITE' == url
        published = true
      end
  ...
  def test_update
    connection_stub = mock()
    connection_stub.expects(:update_request).times(1).then.with do |url, payload|
