...
class Model
  ...
  def self.find( options = nil, custom_headers = {}, batch = nil, is_anon = false )
    ...
    if options.is_a?(String)
      path = "/#{service_name}/#{options}.json"
  ...
  def self.update_attributes!(id, attributes)
    ...
    parse(CoreServer::Base.connection.update_request(path, attributes.fix_key_encoding.to_json))
  end
  ...
  def self.create(attributes, custom_headers = {})
    ...
    parse(CoreServer::Base.connection.create_request(path, attributes.to_json, custom_headers))
  end

  def self.parse(data)
    return if data.blank?

    set_up_model(JSON.parse(data, :max_nesting => 25))
  end
...
protected
  ...
  def flags_modified?
    @added_flags.present? || @deleted_flags.present?
  end
