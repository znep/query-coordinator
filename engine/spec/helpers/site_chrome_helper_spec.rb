...
describe SocrataSiteChrome::ApplicationHelper do
  ...
  describe '#cleanse_url' do
    it 'does not modify a url that starts with http://' do
      url = 'http://google.com'
      expect(helper.cleanse_url(url)).to eq('http://google.com')
    end

    it 'does not modify a url that starts with https://' do
      url = 'https://google.com'
      expect(helper.cleanse_url(url)).to eq('https://google.com')
    end

    it 'does not modify a url that starts with /' do
      url = '/browse'
      expect(helper.cleanse_url(url)).to eq('/browse')
    end

    it 'prepends http:// to a url that does not have a scheme or leading slash' do
      url = 'facebook.com/bananas'
      expect(helper.cleanse_url(url)).to eq('http://facebook.com/bananas')

      url2 = 'www.test.com/asdf'
      expect(helper.cleanse_url(url2)).to eq('http://www.test.com/asdf')
    end
    ...
    it 'doesn\'t drop url params and fragments' do
      ...
      expect(helper.cleanse_url(url)).to eq('/browse?some_stuff=true#show')
    end
    ...
    context 'localization' do
      ...
      it 'prepends the current locale to a relative path' do
        ...
        url = '/browse'
        expect(helper.cleanse_url(url)).to eq('/zz/browse')
      end
      ...
      it 'turns a url with the same host as the current domain into a localized relative url' do
        ...
        url = 'https://data.seattle.gov/browse'
        expect(helper.cleanse_url(url)).to eq('/kr/browse')
      end
    end
    ...
    context 'mailto links' do
      it 'does not prepend "http" do a mailto link' do
        ...
        expect(helper.cleanse_url(url)).to eq('mailto:bob@test.com')
      end
    end
  end
