...
$.component.Container.extend('Repeater', 'content', {
    ...
    _loadRows: function(start, count, callback) {
        ...
        if ($.isBlank(cObj._dataContext)) {
            ...
            var dashboardCatItems = cObj._dataContext.dashboard.categories.slice(start, start + count);
            if (!$.isBlank(cObj._properties.groupBy)) {
                renderGroupItems(cObj, dashboardCatItems, callback);
            } else {
                _.each(dashboardCatItems, function(di, i) {
                    if (!_.isObject(di)) { di = { value: di }; }
                    renderedRows.push(cObj._createRow(di, i + start, di));
                });
                callback(renderedRows);
        ...
        } else if ((cObj._dataContext || {}).dataset) {
            ...
            if (cObj._properties.repeaterType == 'column') {
                ...
                _.each(_.select(cObj._dataContext.dataset.visibleColumns, function(c) {
                    if (_.all(exF, function(v, k) { return !_.include($.makeArray(v), $.deepGetStringField(c, k)); }) &&
    ...
    _totalItems: function(callback) {
        ...
        if ($.isBlank(cObj._dataContext)) {
            ...
            if (cObj._properties.repeaterType == 'column') {
                ...
                view.getTotalRows(function() { callback(view.totalRows()); });
            }
        } else if (cObj._dataContext.type == 'datasetList') {
    ...
    _refresh: function() {
        ...
        if (cObj._designing) {
            ...
            if ($.isBlank(cObj._realContainer)) { cObj._setUpRealContainer(); }
    ...
    _render: function() {
        ...
        if (!cObj._updateDataSource(cObj._properties, doRender)) { doRender(); }
    ...
    _propWrite: function(properties) {
        ...
        if (!$.isBlank(children)) { setUpProperties(this, children); }
...
var setUpProperties = function(cObj, children) {
    ...
    function allocateIds(childProperties) {
...
var renderGroupItems = function(cObj, items, callback) {
    ...
    _.each(items, function(item) {
        ...
        if (!$.isBlank(groupConfig.splitOn)) {
            _.each(group.split(groupConfig.splitOn), function(g) { addGroupItem(g); });
        } else {
            addGroupItem(group);
        }
    });

    if (groupConfig.sortAlpha) { groups = groups.sort(); }
