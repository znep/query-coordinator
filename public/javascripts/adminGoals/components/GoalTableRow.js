...
class GoalTableRow extends React.Component {
  ...
  render() {
    ...
    let rowClass = classNames({ selected: selected });
    ...
    return <tr ref='tr' onClick={ this.onClick.bind(this) } className={ rowClass }>
      ...
      <td scope="title">{ this.props.goal.get('name') }
        <span className="goalPageLink externalLink" >
          <Flyout text="Manage on Goal Page">
            <a target="_blank" href={ goalPageUrl }>Goal Page</a>
          </Flyout>
      ...
      <td>
        <Flyout text="View Dashboard">
          <a target="_blank" href={ dashboardUrl }>{ this.props.dashboard.get('name') }</a>
        </Flyout>
      </td>
      <td><a href="" className="goalEditLink">Edit</a></td>
    </tr>;
  }
}
