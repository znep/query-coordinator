/*global google killAnimation animate animated */
(function($) {
  ...
  $.Control.extend('socrataMap', {
    ...
    initializeVisualization: function() {
      ...
      if (mapObj.settings.interactToScroll && $.subKeyDefined(mapObj, '_controls.Navigation') && !mapOptions.disableZoomWheel) {
        $(mapObj.currentDom).one('mouseup', function() {
      ...
      var handleDisplayFormatChange = function() {
        if ((blist.debug || {}).viewport && (console || {}).trace) {
          ...
          console.dir(mapObj._displayFormat);
          console.groupEnd();
          console.groupCollapsed('new state');
          console.dir(this.displayFormat);
          console.groupEnd();
          console.groupEnd();
        }
      ...
      if (mapObj._primaryView) {
        mapObj._primaryView.bind('displayformat_change',
                                 handleDisplayFormatChange).trigger('displayformat_change');
    ...
    updateDisplayFormat: function(df) {
      ...
      _.each(mapObj._children.slice(length), function(childView) {
        childView.destroy();
      });
      mapObj._children = mapObj._children.slice(0, length);
      mapObj._controls.Overview.truncate(length);

      _.each(mapObj._children, function(cv) {
        cv.loading = true;
      });

      var childViewConstructing = false;
      _.each(mapObj._displayFormat.viewDefinitions, function(fmt, index) {
        if (mapObj._children[index] && _.isEqual(mapObj._children[index]._displayFormat, fmt)) {
          delete mapObj._children[index].loading;
          return;
        }

        if (mapObj._children[index] && ($.subKeyDefined(df, 'context.dataset') || df.uid == mapObj._children[index]._view.id || df.uid == 'self' && mapObj._children[index]._view.id == mapObj._primaryView.id) && df.plotStyle == (mapObj._children[index]._displayFormat || {}).plotStyle) {
          mapObj._children[index]._view.trigger('displayformat_change', [df]);
          delete mapObj._children[index].loading;
        } else {
          childViewConstructing = true;
          mapObj._constructDataLayer(df, index);
        }
      });

      if (mapObj._doneLoading && !_.isEqual(
      ...
      if (!childViewConstructing) {
        ...
        if ((mapObj._displayFormat.viewDefinitions || []).length == mapObj._children.length && _.all(mapObj._children, function(cv) {
          return !cv.loading;
        })) {
    ...
    _constructDataLayer: function(displayFormat, index) {
      ...
      if (mapObj._children[index]) {
        mapObj._children[index].destroy();
      }
      ...
      var loadDataset = function(ds) {
        if (df.legacy) {
          if (!ds.isArcGISDataset() && !ds.isGeoDataset()) {
            ...
            if (locCol) {
              df.plot = {
                locationId: locCol.tableColumnId
              };
            }
            df.color = '#0000ff';
          }
        ...
        try {
          mapObj._children[index] = $(mapObj._children[index].$dom).socrataDataLayer(
            $.extend({}, mapObj.layerSettings, {
              view: ds,
              index: index,
              query: query,
              parentViz: mapObj,
    ...
    _onDatasetsLoaded: function() {
      var mapObj = this;
      mapObj._children = _.reject(mapObj._children, function(cv) {
        return cv.invalid;
      });
      ...
      if (_.any(mapObj._invalidChildren,
                function(cv) {
                  return _.isString(cv.error) && cv.error.indexOf('heat map') > -1;
                })) {
      ...
      if (mapObj._primaryView) {
        mapObj._primaryView.childViews = _.chain(mapObj._children).map(function(c) {
          if (c._view.childViews) {
            return c._view.childViews;
          } else {
            return c._view.id;
          }
        }).flatten().uniq().value();
      ...
      if (mapObj._primaryView) {
        mapObj._primaryView.trigger('row_count_change');
      } // DEBUG EZMODE Sidebar ready.

      mapObj.viewportHandler().events.register('viewportchanged', null,
                                               function() {
                                                 mapObj._panning = true;
                                               });
    },
    ...
    restackDataLayers: function() {
      ...
      var views = _.sortBy(mapObj._children, function(cv) {
        return cv._index;
      });
    ...
    mapElementLoaded: function(layer) {
      ...
      if (blist.openLayers.isBackgroundLayer(layer)) {
        layer._loaded = true;
      }

      if (!mapObj._doneLoading && (mapObj.map.hasNoBackground || _.chain(mapObj.map.backgroundLayers()).pluck('_loaded').all().value()) && _.chain(mapObj._children).invoke('ready').all().value()) {
    ...
    initializeBackgroundLayers: function() {
      ...
      if (!mapObj._backgroundLayers || !_.isEqual(mapObj._backgroundLayers, mapObj._displayFormat.bkgdLayers)) {
      ...
      _.each(bkgdLayers, function(layer) {
        mapObj.addBackgroundLayer(layer);
      });
    ...
    addBackgroundLayer: function(layerOptions) {
      ...
      if (layerOptions.custom_url) {
        ...
        config = _.detect(Dataset.map.backgroundLayers, function(cfg) {
          return cfg.key == layerOptions.layerKey;
        });
      }
      ...
      switch (config.className) {
        case 'Google':
          options = $.extend(options, {
            type: google.maps.MapTypeId[options.type]
          });
          layer = new OpenLayers.Layer.Google(config.key, options);
          break;
        case 'ESRI':
          var url = config.key == 'custom' ? config.custom_url : 'https://server.arcgisonline.com/ArcGIS/rest/services/' + options.url + '/MapServer';
          var name;
          if (config.key == 'custom') {
            name = url.match(/services\/(.*)\/[A-Za-z]+Server/);
            if (name) {
              name = name[1];
            }
          }
      ...
      var hideLayer;
      if (!mapObj._displayFormat.exclusiveLayers || !mapObj.map.baseLayer || layer instanceof OpenLayers.Layer.Google) {
        if (mapObj._viewportHandler) {
      ...
      if (layerOptions.hidden || hideLayer) {
    ...
    saveQuery: function(uid, query) {
      ...
      this._primaryView.update({
        metadata: newMD
      });
      if ((blist.debug || {}).events && (console || {}).trace) {
        console.groupCollapsed('saveQuery (after)');
        console.groupCollapsed('_primaryView');
        console.dir(this._primaryView.metadata);
        console.groupEnd();
        console.groupCollapsed('blist.dataset');
        console.dir(blist.dataset.metadata);
        console.groupEnd();
        console.groupEnd();
      }
    },
    ...
    initializeEvents: function() {
      ...
      mapObj.map.events.register('zoomend', null, function() {
        ...
        _.each(mapObj.map.backgroundLayers(), function(layer) {
          if (zoomLevel >= layer.zoomLevels) {
            layer.setVisibility(false);
            layer.hiddenByZoom = true;
          } else if (layer.hiddenByZoom) {
            layer.setVisibility(true);
            delete layer.hiddenByZoom;
          }
        });
        mapObj._controls.Overview.redraw();
      });
      ...
      mapObj.map.getControlsByClass('OpenLayers.Control.Attribution')[0].events.
             register('attributionupdated', null,
                      function() {
                        mapObj._controls.Overview.correctHeight();
                      });

      mapObj.map.events.register('mousemove', mapObj,
                                 function(evt) {
                                   mapObj._lastClickAt = mapObj.map.events.getMousePosition(evt);
                                 });
    },
    ...
    buildSelectFeature: function() {
      ...
      var featureLayers = _.chain(mapObj._children).
                            map(function(childView) {
                              return childView.selectableFeatureLayers();
                            }).
                            compact().
                            flatten().
                            value();
    ...
    viewportHandler: function() {
      var mapObj = this;
      if (!mapObj._viewportHandler) {
        mapObj.map.addControl(mapObj._viewportHandler = new blist.openLayers.Viewport(mapObj, mapObj._displayFormat.viewport));
      }
    ...
    rowsLoaded: function(num) {
      if (_.isUndefined(this._rowsLoaded)) {
        this._rowsLoaded = 0;
      }
    ...
    resizeHandle: function() {
      ...
      if (mapObj.map) {
        mapObj.viewportHandler().expect('resizeHandle');
      }
      if (mapObj._controls && mapObj._controls.ZoomBar) {
        mapObj._controls.ZoomBar.redraw();
      }
    ...
    getDataForChildren: function() {
      ...
      if (_.any(mapObj._children, function(childView) {
        return childView.loading;
      })) {
        return;
      }
    ...
    initializeAnimation: function(data, view) {
      ...
      if (mapObj._displayFormat.plotStyle != 'point') {
        return;
      }
      if (_.isEmpty(data)) {
        return;
    ...
    runAnimation: function(callback) {
      ...
      var direction = _.detect(mapObj._byView, function(viewConfig) {
        return viewConfig._animation && !viewConfig._animation.finished && viewConfig._animation.direction != 'none';
      });
      ...
      if (!direction) {
        _.each(mapObj._byView, function(viewConfig) {
          if (_.isFunction(viewConfig._displayLayer.removeFeatures) && viewConfig._animation) {
      ...
      var animations = _.reduce(mapObj._byView, function(memo, viewConfig) {
        ...
        return memo.concat(_.compact(_.map(
          viewConfig._animation[animKey],
          function(feature) {
            if (!feature.attributes.clusterParent) {
              return;
            }
            ...
            if (direction == 'spread') {
              animation.from = otherNode ? otherNode.geometry.getBounds().getCenterLonLat() : otherNodeLonLat;
              animation.to = feature.geometry.getBounds().getCenterLonLat();
            } else {
              animation.to = otherNode ? otherNode.geometry.getBounds().getCenterLonLat() : otherNodeLonLat;
      ...
      setTimeout(function() {
        killAnimation = true;
        if (_.isFunction(window.killingAnimations)) {
          window.killingAnimations();
        }
    ...
    showPopup: function(lonlat, contents, options) {
      ...
      $('.olFramedCloudPopupContent').on('click', '.infoPaging a', function(event) {
