  ...
  class Order
    def self.validate_order_by(order_bys, metadata)
      ...
      order_bys.each do |o|
        ...
        Util.resolve_field_type(field, nil, metadata) || fail("Unable to resolve field type for #{field} in order-by", o)
      end
    end
    ...
    def self.order_rows(rows, order_bys, metadata)
      ...
      rows.sort do |a, b|
        ...
        begin
          ...
          type = Util.resolve_field_type(field, nil, metadata) || fail("Unable to resolve field type for #{field} in order-by", order_bys[i], true)
