export const home = routing => {
  ...
  const matches = path.match(/^\/(\w{2}\/)?\w+\/[\w-]+\/\w{4}-\w{4}\/revisions\/\d+/);
  return matches[0];
};
...
export const showOutputSchema = (uploadId, inputSchemaId, outputSchemaId, pageNo) => routing =>
...
export const showRowErrors = (uploadId, inputSchemaId, outputSchemaId, pageNo) => routing =>
  `${home(routing)}/uploads/${uploadId}/schemas/${inputSchemaId}/output/` +
