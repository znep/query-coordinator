...
import * as FlashActions from 'actions/flashMessage';
import styles from 'styles/Modals/GeocodeShortcut.scss';
import flashMessageStyles from 'styles/FlashMessage/FlashMessage.scss';
import { ADD_COLUMN } from 'actions/apiCalls';
...
export class GeocodeShortcut extends Component {
  ...
  relevantOutputColumnsForComposition() {
    const outputColumnsOf = (mappingNames) => this.state.mappings
  ...
  genDesiredColumns() {
    ...
    if (this.state.shouldHideOriginal) {
      const columnIdsToHide = this.relevantOutputColumnsForComposition()
        .filter(oc => !!oc)
        .map(oc => oc.id);
  ...
  createNewOutputSchema() {
    return this.props.newOutputSchema(
      this.props.path.inputSchemaId,
  ...
  render() {
    ...
    const latlngClassname = classNames(
      { [styles.compositionSelected]: isLatLng, [styles.compositionButton]: !isLatLng }
    );
    const composeComponents = () => this.setState({ ...this.state, composedFrom: COMPONENTS });
    const componentsClassname = classNames(
      { [styles.compositionSelected]: isComponents, [styles.compositionButton]: !isComponents }
    );
    const composeCombined = () => this.setState({ ...this.state, composedFrom: COMBINED });
    const combinedClassname = classNames(
      { [styles.compositionSelected]: isCombined, [styles.compositionButton]: !isCombined }
    );
...
const mergeProps = (stateProps, { dispatch }, ownProps) => {
  ...
  const dispatchProps = {
    ...
    showError: (message) =>
      dispatch(FlashActions.showFlashMessage('error', message, 3500))
  };
