(function($) {
    ...
    prototype: {
      ...
      sort: function(ascending) {
        ...
        if ($.isBlank(ascending)) {
          ...
          query.order = [{
            columnFieldName: cmObj.settings.column.fieldName,
            ascending: ascending
          }];
        }

        cmObj.settings.view.update({
            metadata: md
          }, false,
  ...
  var canFilter = function(cmObj, col) {
    ...
    return $.isBlank(col.parentColumn) && !cmObj.settings.view.isGrouped() &&
      _.any(types, function(t) {
        return $.subKeyDefined(t, 'filterConditions.details.EQUALS');
      });
  };

  /* Hook up JS behavior for menu.  This is safe to be applied multiple times */
  var hookUpHeaderMenu = function(cmObj) {
    cmObj.$menu.dropdownMenu({
        triggerButton: cmObj.settings.$menuTrigger,
        openCallback: function() {
          columnMenuOpenCallback(cmObj);
        },
        linkCallback: function(e) {
          columnHeaderMenuHandler(cmObj, e);
        },
        forcePosition: true,
        pullToTop: true
      }).
  ...
  var addFilterMenu = function(cmObj, summary) {
    ...
    if (!$.isBlank(cmObj.settings.column.currentFilter)) {
      menuItem.contents.push({
        tagName: 'li',
        'class': 'clearFilter',
        contents: {
          tagName: 'a',
          href: '#clear-filter-column',
          contents: {
            tagName: 'span',
            'class': 'highlight',
    ...
    menuItem.contents.push({
      ...
      contents: {
        tagName: 'a',
        href: '#pref',
        title: $.t('controls.grid.previous'),
        contents: {
          tagName: 'div',
          'class': 'outerWrapper',
          contents: {
    ...
    _.each(typeKeys, function(k) {
      ...
      if (!$.isBlank(cs.topFrequencies)) {
        ...
        _.each(cs.topFrequencies, function(f) {
          f.isMatching = !$.isBlank(cmObj.settings.column.currentFilter) &&
  ...
  var deleteColumns = function(cmObj, columns) {
    ...
    if (confirm(multiCols ? $.t('controls.grid.delete_warning_multi_column') : $.t('controls.grid.delete_warning_single_column'))) {
      if (_.isEmpty(columns) &&
        ...
        cmObj.settings.column.parentColumn.removeChildColumns(cmObj.settings.column.id);
      } else {
        if (_.isEmpty(columns)) {
