...
$(function() {
  ...
  // USER TRACKING
  var mixpanelEvents = {
    started: 'Ingress: Started Wizard Page',
    left: 'Ingress: Left Wizard Page'
  };
      ...
      'working': {
        ...
        onActivate: function($pane, config, state, command) {
          ...
          if (viewData.metadata && state.submittedView && state.submittedView.metadata) {
            $.extend(true, viewData, {
              metadata: state.submittedView.metadata
            });
          }
          if (viewData.privateMetadata && state.submittedView && state.submittedView.privateMetadata) {
            $.extend(true, viewData, {
              privateMetadata: state.submittedView.privateMetadata
            });
          }
