...
describe('SvgRegionMap', function() {
   ...
   */
  beforeEach(function() {
    testData = window.svgRegionMapTestData;
      ...
      describe('1-2 values', function() {
        ...
        describe('continuous legend', function() {
          ...
          it('colors datasets with only one positive value blue', function() {
            ...
            var legendColor = chroma.color(gradient.css('stop-color')).rgb();

            expect(isBlue(legendColor)).to.equal(true);
          });
    ...
    describe('legend', function() {
      ...
      describe('discrete scale', function() {
        ...
        it('should contain labels that are evenly spaced numbers', function() {
          ...
          var isEquidistant = _.reduce(
            offsets,
            function(difference, offset, i) {
        ...
        it('should emit an event upon hover over a legend color', function(done) {
          ...
          el.on('SOCRATA_VISUALIZATION_REGION_MAP_LEGEND_FLYOUT', function(event) {
      ...
      describe('continous scale', function() {
        ...
        describe('all positive values', function() {
          ...
          it('creates a logarithmic scale for large ranges > 0', function() {
            ...
            expect(largeTicks.length).to.equal(3);
            expect(values[Math.ceil(values.length / 2)].value).to.be.greaterThan(toNumber(largeTicks.eq(1).text()));
        ...
        describe('all negative values', function() {
          ...
          it('creates a logarithmic scale for large ranges', function() {
            ...
            expect(largeTicks.length).to.equal(3);
            expect(values[Math.floor(values.length / 2)].value).to.be.lessThan(toNumber(largeTicks.eq(1).text()));
          });
        });
        ...
        describe('changing values', function() {
          ...
          it('rescales the legend for the new value', function() {
            ...
            var newGeojsonAggregateData = aggregateDataForValues(vif, values);

            regionMapObject.visualization.render(regionMapObject.vif, newGeojsonAggregateData)
