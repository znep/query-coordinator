...
function choropleth(
  ...
  function LegendDiscrete(element, container, scope) {
    ...
    FlyoutService.register({
      ...
      render: function(flyoutElement) {
        ...
        return `<div class="flyout-title">${flyoutElement.getAttribute('data-flyout-text')}</div>`;
      },
  ...
  $.extend(LegendContinuous.prototype, {
    ...
    drawAxis: function(ticksSvg, gradientSvg, tickStops, scale, indexOfZero) {
      ...
      ticksSvg.css('top', `${parseInt(gradientSvg.css('top'), 10) - tickMaxHeight / 2}px`);

      // Now listen to me very carefully. Compensate for shift in SVG by putting the ticks back.
      ticksGroup.attr('transform', `translate(${tickMaxWidth},${tickMaxHeight / 2})`);
    },
