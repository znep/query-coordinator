...
$(function() {
  ...
  blist.nominations.friendlyStatus = function(nom) {
    if (nom.status == 'pending' && (new Date(nom.createdAt * 1000) > Date.parse('7 days ago'))) {
  ...
  blist.nominations.remove = function(id, successCallback) {
    ...
    if (confirm($.t('controls.nominate.delete_confirm', {
        type: type
      }))) {
      blist.nominations.map[id].remove(successCallback,
        function error(xhr) {
          alert($.t('controls.nominate.delete_error', {
              type: type
            }) + ': ' +
