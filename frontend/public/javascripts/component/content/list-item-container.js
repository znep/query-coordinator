(function($) {
  ...
  $.component.Container.extend('List Item Container', 'none', {
    ...
    _getContainer: function() {
      if ($.isBlank(this.$ct)) {
        ...
        if (this.$ct.length != 1) {
          this.$ct = $.tag({
            tagName: tag,
            'class': this._stringSubstitute(this._properties.listCustomClass)
          });
    ...
    _moveChildDom: function(child) {
      ...
      if ($.subKeyDefined(child, 'next.$wrapper') && child.next.$wrapper.parent().index(this.$ct) >= 0) {
    ...
    _arrange: function() {
      var cObj = this;
      if (cObj._blockArrange) {
        return;
      }

      cObj.each(function(child) {
        if ($.isBlank(child.$wrapper)) {
          return;
        }
  ...
  var getItemTag = function(cObj, child) {
    return child._stringSubstitute(cObj._properties.itemTag) ||
      { ul: 'li', ol: 'li', dl: 'dd' }[getListTag(cObj)] ||
      'div';
  };
