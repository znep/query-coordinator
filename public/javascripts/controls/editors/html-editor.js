...
(function($) {
  ...
  var createRTE = function() {
    ...
    var VALID_ELEMENTS = '@[id|class|style|title|dir<ltr?rtl|lang|xml::lang],a[rel|rev|charset|hreflang|tabindex|accesskey|type|' +
  ...
  var booleanCommand = {
    query: function(editor, name) {
      return {
        enabled: editor.getDoc().queryCommandEnabled(name),
        value: editor.queryCommandState(name)
      };
    },
  ...
  var valueCommand = {
    query: function(editor, name) {
      return {
        enabled: editor.getDoc().queryCommandEnabled(name),
        value: editor.queryCommandValue(name)
      };
    },
