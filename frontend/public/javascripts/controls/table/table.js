if (!blist.feature_flags.enable_2017_grid_view_refresh) {
  ...
  (function($) {
    ...
    var begin = trace ? function(what) {
      console.time(what);
    } : function() {};

    // Note exit from a block
    var end = trace ? function(what) {
    ...
    var makeTable = function(options) {
      ...
      var selectColumnTo = function(column) {
        ...
        _.each(columns, function(c) {
          ...
          if (inSelect) {
            cellNav.setColumnSelection(c, true);
          }
        });
        selectColumn(column, true);
      };
      ...
      var getRenderedRowsWithPosition = function() {
        ...
        for (var rowId in renderedRows) {
          var row = renderedRows[rowId];
          rows.push(row);
        }
        return rows = _.sortBy(rows, function(r) {
          return model.index(r);
        });
      };
      ...
      var expandActiveCell = function() {
        ...
        if ($activeCells && $activeCells.length > 0) {
          ...
          if (type && type.isInlineEdit) {
            if (editCell($activeCells[0], SELECT_EDIT_MODE)) {
              return;
            }
          }
        }
        ...
        if (!$activeContainer) {
          ...
          $activeContainer[0].parentNode.nodeType == 11) {
          // doc fragment
        ...
        if (row !== undefined && row.expanded) {
          ...
          $activeContainer.removeClass('blist-tr-open');
        }
        if (!$activeCells) {
      ...
      var cellXY = function(cell) {
        ...
        var node;
        var lcol;
        for (x = 0, node = cell.parentNode.firstChild; node; node = node.nextSibling) {
          lcol = rowLayout[x];
          if (!lcol) {
            break;
          }
      ...
      var editCell = function(cell, mode, newValue) {
        ...
        if ($.isBlank(realCol.renderType.interfaceType)) {
          return;
        }
        $curEditContainer.blistEditor({
      ...
      var configureEditor = function(cell, $curEditContainer, mode) {
        ...
        var displayCallback = function() {
          resizeEditor();
          $curEditContainer.bind('resize', function() {
            resizeEditor();
          });
      ...
      var expandHotCell = function() {
        ...
        if (type && type.isInlineEdit) {
          if (editCell(hotCell, EXPAND_EDIT_MODE)) {
            return;
          }
        }
        ...
        if (hotExpander.parentNode == null ||
          hotExpander.parentNode.nodeType == 11) {
      ...
      var positionCellOverlay = function($container, $refCell, animate, curSize) {
        ...
        if (left + contWidth > scrollLeft + viewportWidth &&
          ...
          maxWidth = scrollLeft + viewportWidth - left;
        }
        ...
        if (hh) {
          if (hh != hotHeader || hhm != hotHeaderMode) {
            ...
            if (hotHeaderMode == 2) {
              ...
              if (isNT) {
                $hh.css('cursor', 'col-resize');
              }
            } else {
              $outside.css('cursor', 'default');
              if (isNT) {
                $hh.css('cursor', 'default');
              }
            }
          }
      ...
      var handleColumnResize = function(event, isFinished) {
        ...
        if (col.hasOwnProperty('percentWidth')) {
          ...
          variableColumns[0] = $.grep(variableColumns[0], function(c) {
            return c.id == col.id;
          }, true);
          if (col.minWidth) {
      ...
      var onCellClick = function(event, origEvent) {
        ...
        if (cell) {
          ...
          // Notify listeners
          var cellEvent = $.Event('cellclick'); //eslint-disable-line new-cap
      ...
      var onTouchMove = function(event) {
        ...
        var $outsideScrolls = $outside.find('.blist-table-scrolls');

        var deltaY = touchLastY - e.targetTouches[0].clientY;
        // in reality scrolling exactly with detection runs ~2x speed
        deltaY /= 2;
        $outsideScrolls[0].scrollTop = $outsideScrolls[0].scrollTop + deltaY;

        var deltaX = touchLastX - e.targetTouches[0].clientX;
        $outsideScrolls[0].scrollLeft = $outsideScrolls[0].scrollLeft + deltaX;
      };
      ...
      var onMouseUp = function(event) {
        ...
        var $target = $(event.target);
        if ($target.closest('.action-item').length > 0) {
          return;
        }
        if ($target.parents().index($outside) < 0) {
          return;
        }
        ...
        if (!isDisabled && cellNav && !editMode) {
          expandActiveCell();
        }
      };
      ...
      if (options.simpleCellExpand) {
        $('.blist-td:not(.blist-td-popout)').live('mouseover', function() {
          var $td = $(this);

          var innerContentWidth = 0;
          $td.children().each(function() {
            innerContentWidth += $(this).outerWidth(true);
          });

          if (innerContentWidth <= $td.innerWidth()) {
            return;
          }

          var offsetPos = $td.offset();
          offsetPos.top += $td.offsetParent().scrollTop();

          var $copy = $td.clone();
          var outTimer;
          var clearCopy = function() {
            $copy.stop().fadeOut('fast', function() {
              $copy.remove();
            });
          };
          $copy.
      ...
      var renderCell = function(html, index, renderIndex, row, col, colIndex, level, contextVariables) {
        ...
        if (!$.isBlank(col.visibleChildColumns) && level == 0) {
          children = col.visibleChildColumns;
          ...
          html.push('<div class="blist-td ', getColumnClass(col), cls,
            ...
            (curRow.sessionMeta && curRow.sessionMeta.highlightColumn == col.id ?
              ' blist-td-highlight' : ''));
          if ($.isBlank(col.parentColumn)) {
            html.push(' ', ((contextVariables.cellClasses[row.id] || {})[col.lookup] || []).join(' '));
          }
          html.push('">');
      ...
      var initMeta = function() {
        ...
        if (options.showRowNumbers) {
          lockedColumns.push(rowNumberColumn = {
            id: 'rowNumberCol',
        ...
        if (options.showRowHandle) {
          lockedColumns.push(rowHandleColumn = {
            id: 'rowHandleCol',
        ...
        var measuredInnerDims = {
          width: Math.max(0, Math.floor($measureDiv.width())),
          height: Math.max(0, $measureDiv.height())
        };
        var measuredOuterDims = {
        ...
        for (i = 0; i < model.columns().length; i++) {
          var modelColumns = model.columns()[i];
          var layoutColumns = layout[i] = [];
          processLogicalColumns(modelColumns, layoutColumns, i);
        }
        ...
        rowLockedRenderFn = function(html, index, renderIndex, row) {
          ...
          rowDivContents(html, index, renderIndex, row);
          html.push('>');
          for (i = 0; i < lockedColumns.length; i++) {
      ...
      var configureLevelWidths = function(mcols, level) {
        ...
        for (var j = 0; j < mcols.length; j++) {
          ...
          if (colWidth) {
            ...
            try {
              columnStyle = getColumnStyle(mcol);
            } catch (e) {
              continue;
            }
            ...
            if (columnStyle.width != widthStyle) {
      ...
      var renderHeader = function() {
        ...
        for (var i = 0; i < columns.length; i++) {
          ...
          html.push(
            '<div class="blist-th ', !i ? 'blist-th-first ' : '',
        ...
        var lockedHtml = '';
        $.each(lockedColumns, function(index, c) {
      ...
      var adjustHeaderStyling = function($colHeader, tertiary) {
        ...
        if (infoW + 20 > innerW) {
          ...
          if (infoW > innerW) {
            ...
            $orig.removeClass('narrow narrower');
          }
        } else if (tertiary) {
          $orig.removeClass('narrow narrower');
        }
      ...
      var renderFooter = function() {
        ...
        var gotAggs = function() {
          var html = [];
          var showAgg = false;
          var renderColFooter = function(column) {
            var cls = column.cls ? ' blist-tf-' + column.cls : '';
            var agg = column.aggregates[column.format.aggregate];
            ...
            if (!$.isBlank(agg) &&
              _.include(['sum', 'average', 'maximum', 'minimum'],
                column.format.aggregate)) {
              var c = column;
              if (column.format.aggregate == 'average') {
                c = $.extend(true, {
                  format: {
                    precision: 3
                  }
              ...
              val = column.renderType.renderer(agg, c);
            }

            html.push(
              '<div class="blist-tf ', !i ? 'blist-tf-first ' : '',
              getColumnClass(column),
              cls,
              '" title="',
              (column.format.aggregate || '').capitalize(),
              '" colId="',
              column.id,
              '">',
        ...
        if (model.getAggregates(function() {
            _.defer(gotAggs);
          })) {
          pendingAggs = true;
        }
      ...
      var currentPageText = function() {
        return $.t('controls.grid.viewing_current_page', {
          current: $.commaify(model.currentBucketIndexOrUndefined() + 1),
          total: $.commaify(model.totalBuckets())
        });
      };

      var updateCurrentPageText = function() {
      ...
      var handlePageChange = function(pageChangeOptions) {
        if ((pageChangeOptions || {}).goToBottom) {
          scrollToBottom();
        } else {
          scrollToTop();
        }
      ...
      var renderPagination = function() {
        var $outsidePagination = $outside.find('.blist-table-pagination');

        // If we're not using buckets, then there's no pagination.
        if (!model.usingBuckets()) {
          $outsidePagination.hide();
          return;
        } else {
          $outsidePagination.show();
        }
        ...
        if (model.totalBuckets() <= 1) {
          $outsidePagination.find('li:not(.rowCount)').remove();
        }
        ...
        $('.button', $outsidePagination).click(function(e) {
          var $button = $(this);
          e.preventDefault();
          ...
          switch ($.hashHref($button.attr('href'))) {
            case 'start':
      ...
      var appendRows = function(html) {
        ...
        var appendRowsRender = function() {
          appendUtilDOM.innerHTML = html;
        };
        var appendRowsAppend = function() {
      ...
      var renderRows = function() {
        ...
        var cleanRow = function(dirtyRowId, rowSet) {
          if (!rowSet[dirtyRowId]) {
            return;
          }
          var row = rowSet[dirtyRowId].row;
          if (row !== undefined) {
            row.parentNode.removeChild(row);
          }
          row = rowSet[dirtyRowId].locked;
          if (row !== undefined) {
            row.parentNode.removeChild(row);
          }
          delete rowSet[dirtyRowId];
        };
        ...
        var rowsLoaded = function(rows) {
          ...
          if (options.rowMods !== null) {
            options.rowMods(renderedRows);
          }
      ...
      $this.bind('dataset_ready.table', function(event, newModel) {
        ...
        var isReady = function() {
          ...
          model.view.bind('row_change', function(rows) {
            ...
            bind('column_totals_changed', function() {
