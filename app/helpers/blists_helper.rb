module BlistsHelper
  ...
  def dataset_menu_lookup(view, is_widget = false, theme = nil)
    ...
    if @view.is_blobby? || @view.is_href?
      options['download'] =  {'text' => 'Download', 'class' => 'export noExportIcon',
        'href' => @view.dataset_href, 'external' => true}
    else
  ...
  def rdf_class_display_name(selected_rdf_class)
    if (selected_rdf_class.nil? || selected_rdf_class == '')
