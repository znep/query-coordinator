  ...
  return {
    ...
    link: function($scope, element) {
      ...
      function setupTable() {
        ...
        $('body').on(`click.${instanceUniqueNamespace}`, '.flyout .caret', function(e) {
        ...
        var getFormattedSort = function() {
          return (_.isPresent(sortColumnId) && _.isPresent(sortOrdering)) ?
            `${SoqlHelpers.formatFieldName(sortColumnId)} ${sortOrdering}` :
            '';
        };
        ...
        var dragHandles = function() {
          ...
          $('body').on(`mousemove.${instanceUniqueNamespace}`, function(e) {
            if (columnDrag) {
              var $cells = $table.find(`.cell:nth-child(${columnIndex + 1}), .th:nth-child(${columnIndex + 1})`);
          ...
          }).on(`mouseup.${instanceUniqueNamespace}`, function() {
            columnDrag = false;
          });
        };
        ...
        var updateColumnHeaders = function() {
          ...
          if (_.isPresent(columnId)) {
            $tableHead.find(`.th:contains(${getColumn(columnId).title})`).mouseenter();
          }
        };
