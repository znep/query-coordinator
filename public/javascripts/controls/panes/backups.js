  ...
  var renderSection = function(cpObj) {
    ...
    cpObj._renderBlock = function(skipAnimation) {
      ...
      _.each(cpObj._backupList.slice(rendered, rendered + PAGE_SIZE), function(backup) {
        var $li = $.renderTemplate('backupItemContainer', {
          backup: backup
        }, {
          ...
          '.deleteBackupLink@class+': function() {
            var hasDeleteRight = _.include(cpObj._view.rights, blist.rights.view.DELETE_VIEW);
            return hasDeleteRight ? '' : 'hide';
          }
        });

        $li.data('backupURI', backup.downloadLinks.csv);
        ...
        customContent: {
          ...
          callback: function($s) {
            ...
            var fetchBackups = function() {
              cpObj._view.getBackups(function(backups) {
                if (processing) {
                  cpObj._finishProcessing();
                  processing = false;
                }

                cpObj._backupList = backups;
            ...
            if (cpObj._view.hasRight(blist.rights.view.DELETE_VIEW)) {
              cpObj._$section.on('click', '.deleteBackupLink', function() {
                var backupURI = $(this).parents('li').data('backupURI');
                cpObj._view.deleteBackup(backupURI, fetchBackups);
              });
            }
