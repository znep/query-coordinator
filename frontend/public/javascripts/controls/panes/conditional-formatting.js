(function($) {
  ...
  var filterOperators = function(vals) {
    ...
    if ($.isBlank(type.filterConditions)) {
      return null;
    }
    return _.map(type.filterConditions.orderedList,
      function(op) {
        return {
          value: op,
          text: type.filterConditions.details[op].text
        };
      });
  };

  var filterEditor = function($field, vals, curValue) {
    if ($.isBlank(vals.tableColumnId) || $.isBlank(vals.operator)) {
      return false;
    }
    ...
    _.times(fc.editorCount, function(i) {
      ...
      $editor.blistEditor({
        ...
        row: null,
        value: _.isArray(curValue) ? curValue[i] : curValue,
        format: col.format,
        customProperties: cp
      });
      $field.append($editor);
  ...
  var filterEditorCleanup = function($field) {
    ...
    $editor.each(function() {
      var $t = $(this);
      if ($t.isControlClass('blistEditor')) {
        $t.blistEditor().finishEdit();
      }
    });
  };
        ...
        create: function($field, vals, curValue) {
          ...
          $field.find('a').data('ajaxupload', new AjaxUpload($field, {
            ...
            onComplete: function(file, response) {
              ...
              $field.parents('.radioLine').find('input:radio').prop('checked', true).click();
            }
          }));
          return true;
        },
  ...
  $.Control.extend('pane_conditionalFormatting', {
    setView: function(newView) {
      ...
      var handler = function(ds) {
        ...
        cpObj._view.bind('conditionalformatting_change', function() {
          cpObj.reset();
        }, cpObj);
      ...
      if ($.subKeyDefined(newView, 'displayFormat.viewDefinitions')) {
        ...
        cpObj._parentView.bind('clear_temporary', function() {
          cpObj.reset();
        }, cpObj);
        ...
        cpObj._parentView.bind('conditionalformatting_change', function() {
          cpObj.reset();
        }, cpObj);
    ...
    _getSections: function() {
      return [{
        ...
        fields: [{
          type: 'repeater',
          minimum: 0,
          addText: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.new_rule_button'),
          name: 'metadata.conditionalFormatting',
          field: {
            type: 'group',
            extraClass: 'conditionGroup',
            options: [{
                type: 'text',
                text: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.description'),
                name: 'description',
                prompt: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.description_prompt')
              },
              conditionIndicator, {
                type: 'select',
                text: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.when_title'),
                prompt: null,
                options: [{
                  text: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.when.all'),
                  value: 'and'
                }, {
                  text: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.when.any'),
                  value: 'or'
                }, {
                  text: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.when.always'),
                  value: 'always'
                }],
                ...
                type: 'repeater',
                minimum: 0,
                addText: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.new_condition_button'),
                ...
                field: {
                  ...
                  options: [{
                    type: 'columnSelect',
                    text: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.item.title'),
                    required: true,
                    name: 'tableColumnId',
                    isTableColumn: true,
                    columns: {
                      type: filterableTypes,
                      hidden: false
                    }
                  }, {
                    type: 'select',
                    ...
                    prompt: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.item.subcolumn'),
                    onlyIf: {
                      field: 'tableColumnId',
                      func: subColumnShown
                    },
                    ...
                    prompt: $.t('screens.ds.grid_sidebar.conditional_formatting.conditions.item.operator'),
    ...
    _finish: function(data, value, finalCallback) {
      ...
      if (cpObj._parentView) {
        ...
        parentMD.conditionalFormatting[cpObj._view.id == cpObj._parentView.id ?
          'self' : cpObj._view.id] = (cpObj._view.metadata || {}).conditionalFormatting;
        cpObj._parentView.update({
          metadata: parentMD
        });
      }
