...
// RENDER+EVENTS
    ...
    var renderCondition = function(condition)
    {
            ...
            .find('.menuEntry a').click(function(event)
            {
                ...
                if ($entry.hasClass('toggleAutogenerated'))
                {
                    if (!_.isUndefined(metadata.includeAuto))
                    {
                        ...
                        $filter.find('.line.autogenerated').each(function()
                        {
                            var $line = $(this);
        ...
        else
        {
            ...
            // autogen values
            if (!_.isUndefined(column.cachedContents) && !_.isUndefined(metadata.includeAuto))
            {
    ...
    var addAutogeneratedValues = function(condition, column, $filter, filterUniqueId, usedValues)
    {
            ...
            !_.isUndefined(column.cachedContents.top))
        {
            ...
            internallyUsedValues = usedValues.concat(_.pluck(column.cachedContents.top.slice(0, topCount), 'item'));
                 ...
                 !_.isUndefined(column.cachedContents.largest)) // assume smallest exists
        {
            ...
            if (_.include(['number', 'money', 'percent'], column.renderTypeName))
            {
                ...
                if (range > 5000) // 5000 is somewhat arbitrary but informed by Benford's Law
                {
                    ...
                    rangeBoundaries = _.uniq(rangeBoundaries); // could possibly have a duplicated 0
                }
                else
                {
                    ...
                    var interval = (column.cachedContents.largest - adjustedLowerBound) / 5;
            ...
            else if (_.include(['date', 'calendar_date'], column.renderTypeName))
            {
                ...
                {
                    if (roundingFactor == 'day')
                    {
                        incrementingDate.setDate(incrementingDate.getDate() + interval);
                    }
                    else if (roundingFactor == 'month')
                    {
                        var month = incrementingDate.getMonth();
                        incrementingDate.setMonth(month + 1);
                        if (month == 11)
                        {
                            incrementingDate.setFullYear(incrementingDate.getFullYear() + interval);
                        }
                        incrementingDate = roundDate(incrementingDate, roundingFactor, 'down');
                    }
                    else if (roundingFactor == 'year')
                    {
                        incrementingDate.setFullYear(incrementingDate.getFullYear() + interval);
                    }
        ...
        if (internallyUsedValues.length > 0)
        {
            // move the edit line back to the end
            $filter.find('.line:not(.autogenerated):last').insertAfter($filter.find('.line:last-child'));
        }
