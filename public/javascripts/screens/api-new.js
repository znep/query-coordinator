$(function() {
  ...
  function makePaneConfig() {
    ...
    onActivate.checkpub = function() {
      $('#progress').hide();
    };

    onActivate.welcome = function($pane, paneConfig) {
      $('#skip').hide();
    ...
    onActivate.datasetResourceName = function() {
      $('#skip').hide();
      var rn = apiView.resourceName;
      if (rn) {
        $('#resourceName').val(rn).blur();
      }
    };
    ...
    onActivate.datasetName = function() {
      var n = apiView.name;
      if (n) {
        $('#editTitle').val(n).blur();
      }
    };
    ...
    onTransition.datasetUniqueId = function($pane, state, callback) {
      ...
      if (newSetting.trim() === '') newSetting = null;
        ...
        _.each(apiView.columns, function(element) {
          if (newSetting === element.fieldName) {
            newSetting = element.id;
          }
        });
      }
      updateView({
    ...
    onActivate.datasetDescription = function() {
      ...
      onTransition[key] = function($pane, state, callback) {
        ...
        if ($i.value()) {
          ...
          if (col.hidden) {
            ...
            col.hide(callback, defaultErrorHandler);
          }
        }
        return false;
      };
    ...
    onActivate.apiPublish = function() {
      $('#skip').hide();
    };

    function apiPublishOnNext() {
      startTransitionUI();
      getApiView(
        function() {
    ...
    onActivate.published = function($pane, paneConfig) {
      $('#skip').hide();
    ...
    onTransition.published = function() {
      $('#paneSpinner').hide();
    ...
    function doWorkBeforeTransition($pane, state, forward) {
      ...
      if (transitionFn) {
        ...
        return (forward ? nextPaneMap[currentPaneId] : 1);
      }
    }
    ...
    function defaultOnActivate($pane, paneConfig, state, command) {
      ...
      $('.nextButton').removeClass('disabled');
      $('.prevButton').removeClass('disabled');
      if (onActivate[currentPaneId]) {
        onActivate[currentPaneId]($pane, paneConfig, state, command);
      }
    }
    ...
    _.each(panes, function(pane, index) {
      ...
      if (index < panes.length - 1) {
        nextPaneMap[key] = panes[index + 1].key;
      }
    });
    stepTotal = ordinal - 1;
    return result;
  ...
  function getDataset(callback, errorCallback) {
    if (blist.configuration.apiFoundry.ds) {
      callback(blist.configuration.apiFoundry.ds);
    }
  ...
  function updateView(changes, callback, errorCallback) {
    if (_.any(changes, function(value, key) {
        return apiView[key] !== value;
      })) {
