(function($) {
  ...
  var regionTypes = [{
    text: $.t('screens.ds.grid_sidebar.map.regions.countries'),
    value: 'countries'
  }, {
    text: $.t('screens.ds.grid_sidebar.map.regions.state'),
    value: 'state'
  }, {
    text: $.t('screens.ds.grid_sidebar.map.regions.canada_provinces'),
    value: 'canada_provinces'
  }, {
    text: $.t('screens.ds.grid_sidebar.map.regions.counties'),
    value: 'counties'
  }];
  var plotStyles = function() {
    // Yes, the names are kinda inaccurate, but eh. We live with it.
    return [{
      text: $.t('screens.ds.grid_sidebar.map.plots.point'),
      value: 'point'
    }, {
      text: $.t('screens.ds.grid_sidebar.map.plots.heatmap'),
      value: 'heatmap'
    }, {
      text: $.t('screens.ds.grid_sidebar.map.plots.rastermap'),
      value: 'rastermap'
    }];
  };
  ...
  var heatmapRegionOptions = function(heatmapType) {
    ...
    var isInvalid = $.isBlank(heatmapType) ||
      ...
      !_.include(heatmapType, 'counties');
    if (isInvalid) {
      return null;
    }
    return [{
  ...
  var errorMessage = function(msg) {
    return $('<span class="error">' + msg + '</span>').css({
      marginLeft: 0,
      paddingLeft: 0
    });
  };
  ...
  var datasetCreate = function($field, vals, curValue) {
    ...
    blist.common.selectedDataset = function(ds) {
      ...
      var createDatasetSuccessCallback = function(dataset) {
        ...
        if (valid) {
          modifySection.call(cpObj, dataset, cpObj._$selectedField);
        }
    ...
    $field.makeStatic = function(value, invalid, uneditable) {
      ...
      $field.find('span.edit').
        click(openSelectDataset).
        css({
          cursor: 'pointer',
          color: '#0000ff'
        });
  ...
  var modifySection = function(dataset, $field) {
    ...
    if (!dataset) {
      if (cpObj.childPanes.length <= index) {
        return;
      }
      cpObj.childPanes.splice(index, 1)[0].$dom().remove();
      _.each(cpObj.childPanes.slice(index), function(cp, i) {
        cp.setIndex(index + i);
      });
      return;
    }
  ...
  mapConfigNS.config = function(options) {
    ...
    if (!options.canvas) {
      config.push({
        ...
        fields: [{
          type: 'repeater',
          name: 'displayFormat.viewDefinitions',
          addText: $.t('screens.ds.grid_sidebar.map.layers.new_data_button'),
          field: {
            type: 'group',
            options: [{
              text: $.t('screens.ds.grid_sidebar.map.layers.dataset'),
              type: 'custom',
              name: 'uid',
              required: true,
              defaultValue: 'self',
    ...
    config.push({
      ...
      fields: [{
        type: 'note',
        value: $.t('screens.ds.grid_sidebar.map.base_layers.subtitle')
      }, {
        type: 'repeater',
        name: 'displayFormat.bkgdLayers',
        addText: $.t('screens.ds.grid_sidebar.map.base_layers.new_base_map_button'),
        minimum: 0,
        field: {
          type: 'group',
          options: [{
            text: $.t('screens.ds.grid_sidebar.map.base_layers.layer'),
            type: 'select',
            name: 'layerKey',
            required: true,
            ...
            text: $.t('screens.ds.grid_sidebar.map.base_layers.opacity'),
            type: 'slider',
            name: 'opacity',
            defaultValue: 1,
            minimum: 0,
      ...
      fields: [{
        type: 'note',
        value: $.t('screens.ds.grid_sidebar.map.advanced.subtitle')
      }, {
        text: $.t('screens.ds.grid_sidebar.map.advanced.exclusive'),
        name: 'displayFormat.exclusiveLayers',
        type: 'checkbox'
      }, {
        text: $.t('screens.ds.grid_sidebar.map.advanced.hide_geolocator'),
        name: 'displayFormat.disableGeolocator',
        type: 'checkbox'
      }]
    }, {
      title: $.t('screens.ds.grid_sidebar.map.legend.title'),
      type: 'selectable',
      fields: [{
        text: $.t('screens.ds.grid_sidebar.map.legend.position'),
        type: 'select',
        prompt: $.t('screens.ds.grid_sidebar.map.legend.position_prompt'),
        defaultValue: 'bottom',
        name: 'displayFormat.legendDetails.position',
        options: [{
          text: $.t('screens.ds.grid_sidebar.map.legend.positions.top_right'),
          value: 'topRight'
        }, {
          text: $.t('screens.ds.grid_sidebar.map.legend.positions.bottom_left'),
          value: 'bottomLeft'
        }, {
          text: $.t('screens.ds.grid_sidebar.map.legend.positions.none'),
          value: 'none'
        }]
      }, {
        text: $.t('screens.ds.grid_sidebar.map.legend.conditional_formats'),
        type: 'checkbox',
        inputFirst: true,
        ...
        field: {
          ...
          options: [{
            type: 'color',
            name: 'color',
            defaultValue: blist.defaultColors,
            lineClass: 'colorCollapse'
          }, {
            text: $.t('screens.ds.grid_sidebar.map.legend.entry_label'),
            type: 'text',
            name: 'label',
            required: true
          }]
  ...
  mapConfigNS.dataLayer.socrataBase = function(options) {
    ...
    return [{
      text: $.t('screens.ds.grid_sidebar.map.data_layer.alias'),
      type: 'text',
      name: prefix + 'alias',
      prompt: $.t('screens.ds.grid_sidebar.map.data_layer.alias_prompt')
    }, {
      text: $.t('screens.ds.grid_sidebar.map.data_layer.plot_style'),
      type: 'select',
      name: prefix + 'plotStyle',
      options: plotStyles,
      required: true,
      prompt: $.t('screens.ds.grid_sidebar.map.data_layer.plot_style')
    }, {
      text: $.t('screens.ds.grid_sidebar.map.data_layer.location'),
      name: prefix + 'plot.locationId',
      type: 'columnSelect',
      isTableColumn: true,
      required: true,
      columns: {
      ...
      text: $.t('screens.ds.grid_sidebar.map.data_layer.region'),
      name: prefix + 'heatmap.type',
      type: 'select',
      onlyIf: boundaryOnly,
      required: !customHeatmap(options),
      prompt: $.t('screens.ds.grid_sidebar.map.data_layer.region_level_prompt'),
      options: regionTypes
    }, {
      text: '',
      name: prefix + 'heatmap.region',
      type: 'select',
      onlyIf: boundaryOnly,
      required: true,
      prompt: $.t('screens.ds.grid_sidebar.map.data_layer.region_prompt'),
      linkedField: prefix + 'heatmap.type',
  ...
  mapConfigNS.dataLayer.socrata = function(options) {
    ...
    return [{
        ...
        text: $.t('screens.ds.grid_sidebar.map.data_layer.highlight_color'),
        name: prefix + 'highlightColor',
        type: 'color',
        onlyIf: pointOnly,
        defaultValue: '#ddeaf0'
      },
      ...
      {
        type: 'note',
        value: $.t('screens.ds.grid_sidebar.map.data_layer.point'),
        onlyIf: pointOnly
      }, {
        text: $.t('screens.ds.grid_sidebar.map.data_layer.point_size'),
        name: prefix + 'plot.sizeValueId',
        onlyIf: pointOnly,
        type: 'columnSelect',
        isTableColumn: true,
        columns: {
        ...
        text: $.t('screens.ds.grid_sidebar.map.data_layer.point_color'),
        name: prefix + 'plot.colorValueId',
        onlyIf: pointOnly,
        type: 'columnSelect',
        isTableColumn: true,
        columns: {
        ...
        text: $.t('screens.ds.grid_sidebar.map.data_layer.icon'),
        name: prefix + 'plot.iconId',
        onlyIf: pointOnly,
        type: 'columnSelect',
        isTableColumn: true,
        columns: {
      ...
      {
        type: 'color',
        text: $.t('screens.ds.grid_sidebar.map.data_layer.color_low'),
        defaultValue: ['#c9c9c9'],
        onlyIf: boundaryOnly,
        ...
        type: 'color',
        defaultValue: ['#00ff00'],
        text: $.t('screens.ds.grid_sidebar.map.data_layer.color_high'),
        onlyIf: boundaryOnly,
      ...
      {
        type: 'note',
        value: $.t('screens.ds.grid_sidebar.map.data_layer.flyout.title'),
        onlyIf: flyouts
      }, {
        text: $.t('screens.ds.grid_sidebar.map.data_layer.flyout.title_column'),
        name: prefix + 'plot.titleId',
        onlyIf: flyouts,
        type: 'columnSelect',
        isTableColumn: true,
        columns: {
          type: ['text', 'location', 'html', 'url',
            'drop_down_list', 'dataset_link', 'email',
            'percent', 'stars', 'flag', 'phone', 'money',
            'data', 'calendar_date', 'number'
          ],
          hidden: options.isEdit,
          defaultNames: ['title']
        }
        ...
        type: 'repeater',
        name: prefix + 'plot.descriptionColumns',
        onlyIf: flyouts,
        field: {
          text: $.t('screens.ds.grid_sidebar.map.data_layer.flyout.details'),
          name: 'tableColumnId',
          type: 'columnSelect',
          isTableColumn: true,
          columns: {
        ...
        minimum: 1,
        addText: $.t('screens.ds.grid_sidebar.map.data_layer.flyout.new_details_button')
      }, {
        text: $.t('screens.ds.grid_sidebar.map.data_layer.flyout.labels'),
        type: 'checkbox',
        name: prefix + 'flyoutsNoLabel',
        onlyIf: flyouts
      },

      {
        text: $.t('screens.ds.grid_sidebar.map.data_layer.flyout.opacity'),
        type: 'slider',
        name: prefix + 'opacity',
        defaultValue: 1,
        minimum: 0,
  ...
  mapConfigNS.dataLayer.mondara = function(options) {
    ...
    return [{
      ...
      text: $.t('screens.ds.grid_sidebar.map.data_layer.opacity'),
      type: 'slider',
      name: prefix + 'opacity',
      defaultValue: 1,
      minimum: 0,
      ...
      type: 'note',
      value: $.t('screens.ds.grid_sidebar.map.data_layer.mondara_note')
    }];
  };
