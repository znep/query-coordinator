$(function() {
  var t = function(str, props) {
    return $.t('screens.admin.users.' + str, props);
  };
  var fadeMessage = function(textValue) {
    $('.userNotice').text(textValue).addClass('notice').removeClass('error').slideDown(300, function() {
      setTimeout(function() {
        $('.userNotice').slideUp();
      }, 5000);
    });
  ...
  $('#adminContent .adminUsersTable .roleSelect').change(function() {
    ...
    $.socrataServer.makeRequest({
      ...
      error: function(request) {
        ...
        $('.userNotice').removeClass('notice').addClass('error').
          text(errorJson.message || t('error_permission_request')).fadeIn(300);
      }
    });
  });
  ...
  $futuresTable.find('.actions .removeButton').adminButton(deleteActions).end().combinationList(comboDefaults);

  var handleResultLength = function($table) {
    $table.toggleClass('noResults', $table.find('tbody tr:visible').length === 0);
  };
  ...
  $('#userRoleFilterDropdown').change(function() {
    ...
    if (filterVal == 'all') {
      ...
      $bothTables.find('tbody tr').hide().filter('[data-userrole=' + filterVal + ']').show();
