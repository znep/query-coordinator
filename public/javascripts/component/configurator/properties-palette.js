(function($) {
  ...
  $.Control.extend('pane_propertiesPalette', {
    _init: function() {
      ...
      if ($.getTemplate('propertiesPaletteContainer').length < 1) {
        $('#js-appended-templates').append($.tag({
          tagName: 'div',
          'class': 'propertiesPaletteContainer',
          contents: {
            tagName: 'span',
            'class': 'cf-property',
            draggable: true
      ...
      if (!comp._updateDataSource(null, reRender)) {
        ...
        if (_.isEmpty(comp._dataContext)) {
          return [{
            fields: [{
              type: 'note',
              value: $.t('dataslate.edit_properties.no_data')
            }]
          }];
        }
  ...
  var sectionConfig = function(prPal, comp, dc) {
    ...
    switch (dc.type) {
      case 'datasetList':
        _.each(dc.datasetList, function(ds) {
          addDatasetProperties(ds, 'datasetList[]');
        });
    ...
    return {
      title: dc.id + ' (' + dc.type.displayable() + ')',
      customContent: {
        ...
        data: {
          dc: dc,
          properties: _.map(_.sortBy(_.keys(availProps), function(k) {
              return availProps[k].shortKey;
            }),
            function(k) {
              return availProps[k];
            })
        },
        callback: function($newSect) {
