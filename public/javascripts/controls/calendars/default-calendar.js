(function($) {
  ...
  var updateQuery = function(calObj, view) {
    ...
    if (!$.isBlank(startDate) && !$.isBlank(endDate)) {
      ...
      if (!$.isBlank(startCol) && !$.isBlank(endCol)) {
        filterCondition.children.push({
          operator: 'AND',
          children: [{
            operator: 'LESS_THAN',
            columnFieldName: startCol.fieldName,
            value: dateToServerValue(startDate, startCol)
          }, {
            operator: 'GREATER_THAN',
            columnFieldName: endCol.fieldName,
  ...
  var viewDisplay = function(calObj) {
    if (calObj._ignoreViewChanges) {
      return;
    }
  ...
  var eventClick = function(calObj, calEvent) {
    ...
    if ($.subKeyDefined(calObj._primaryView, 'highlightTypes.select.' + calEvent.row.id)) {
      ...
      calObj.$dom().trigger('display_row', [{
        row: null
      }]);
  ...
  var renderEventFlyout = function(calObj, calEvent, element) {
    var $e = $(element);
    $e.socrataTip({
      content: calObj.renderFlyout(calEvent.row, calObj._primaryView),
      closeOnClick: false,
      parent: calObj.$dom(),
