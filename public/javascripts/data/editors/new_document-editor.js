(function($)
{
    $.blistEditor.new_document = function(options, dom)
    {
        this.settings = $.extend({}, $.blistEditor.new_document.defaults, options);
        this.currentDom = dom;
        this.init();
    };
    ...
    {
        editObj._curVal = {file_id: fileId, filename: fileName};
        updateButtons(editObj);
    };
    ...
    var updateButtons = function(editObj)
    {
        ...
        $d.find('.docLink')
            .attr('href', v ? editObj.column.base + v.file_id : '')
            .text(v ? v.filename : '');
        $d.trigger('resize');
    };
    ...
    $.extend($.blistEditor.new_document, $.blistEditor.extend(
    {
        prototype:
        {
            $editor: function()
