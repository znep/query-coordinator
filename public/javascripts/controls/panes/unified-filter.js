...
// PLUGIN
    ...
    // RENDER+EVENTS
        ...
        var renderCondition = function(condition)
        {
            ...
            var aggCC = aggregateCachedContents(metadata);
            ...
            $filter.find('.autogeneratedCount').change(function(event)
            {
                ...
                addAutogeneratedValues(condition, aggCC, column, $filter, filterUniqueId, []);
            });
                ...
                .find('.menuEntry a').click(function(event)
                {
                    ...
                    if ($entry.hasClass('toggleAutogenerated'))
                    {
                        ...
                        else
                        {
                            ...
                            var addedValues = addAutogeneratedValues(condition, aggCC, column, $filter, filterUniqueId, []);
            ...
            else
            {
                ...
                if (!_.isUndefined(metadata.includeAuto))
                {
                    addAutogeneratedValues(condition, aggCC, column, $filter, filterUniqueId, usedValues);
                }
        ...
        var aggregateCachedContents = function(metadata)
        {
            var aggCC = {};
            _.each(datasets, function(ds)
            {
                var c = ds.columnForTCID(metadata.tableColumnId[ds.publicationGroup]);
                if (!$.subKeyDefined(c, 'cachedContents')) { return; }
                var cc = c.cachedContents;
        ...
        var addAutogeneratedValues = function(condition, cachedContents, column, $filter, filterUniqueId, usedValues)
        {
            var metadata = condition.metadata || {};
