(function($) {
  var renderStars = function(editObj) {
    editObj.$editor().find('.star').removeClass('star-on').slice(1, editObj.curValue + 1).addClass('star-on');
  };

  var starHover = function(editObj, event) {
    editObj.$editor().addClass('hover').find('.star').removeClass('star-on');
    if ($(event.currentTarget).is('.star-clear')) {
      return;
    }
    $(event.currentTarget).prevAll('.star:not(.star-clear)').andSelf().addClass('star-on');
  };

  var starOut = function(editObj) {
    editObj.$editor().removeClass('hover');
    renderStars(editObj);
  };

  var starClick = function(editObj, event) {
    editObj.curValue = editObj.$editor().
      removeClass('hover').
      find('.star').
  ...
  var starKeyDown = function(editObj, event) {
    if (event.keyCode == 32) {
      if (!_.isNumber(editObj.curValue)) {
        editObj.curValue = 0;
      }
      ...
      if (editObj.curValue > editObj.range()) {
        editObj.curValue = 0;
      }
      if (editObj.curValue < 0) {
        editObj.curValue = editObj.range();
      }
      renderStars(editObj);
  ...
  $.blistEditor.addEditor('stars', {
    ...
    range: function() {
      if (this._range === undefined) {
        ...
        if (this._range <= 0 || isNaN(this._range)) {
          this._range = 5;
        }
      }
      return this._range;
    ...
    querySize: function() {
      return {
        width: this.$editor().find('.star-clear').outerWidth(true) +
