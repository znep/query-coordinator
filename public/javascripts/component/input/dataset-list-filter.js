(function($) {
  ...
  $.component.Component.extend('DatasetListFilter', 'none', { //'input', {
    ...
    _initDom: function() {
      ...
      if ($.isBlank(cObj.$listSection)) {
        ...
        cObj.$listSection.on('click', 'a', function(e) {
          filterClick(cObj, e);
        });
      }
      ...
      if ($.isBlank(cObj.$dialog)) {
        ...
        cObj.$dialog = $.tag2({
          ...
          contents: [{
            _: 'a',
            href: '#close',
            className: ['jqmClose', 'cornerClose'],
            contents: 'Close'
          }, {
            _: 'div',
  ...
  var listOptions = {
    viewTypes: {
      ...
      items: function(callback, cObj) {
        var i = [{
          translation: 'controls.browse.facets.view_types.datasets',
          value: 'datasets',
          className: 'typeBlist'
        }, {
          translation: 'controls.browse.facets.view_types.charts',
          value: 'charts',
          className: 'typeChart'
        }, {
          translation: 'controls.browse.facets.view_types.maps',
          value: 'maps',
          className: 'typeMap'
        }, {
          translation: 'controls.browse.facets.view_types.calendars',
          value: 'calendars',
          className: 'typeCalendar'
        }, {
          translation: 'controls.browse.facets.view_types.filters',
          value: 'filters',
          className: 'typeFilter'
        }, {
          translation: 'controls.browse.facets.view_types.href',
          value: 'href',
          className: 'typeHref'
        }, {
          translation: 'controls.browse.facets.view_types.blob',
          value: 'blob',
          className: 'typeBlob'
        }, {
          translation: 'controls.browse.facets.view_types.forms',
          value: 'forms',
          className: 'typeForm'
        }];
        var apiItem = {
          translation: 'controls.browse.facets.view_types.apis',
          value: 'apis',
          className: 'typeApi'
        };
    ...
    topics: {
      ...
      items: function(callback, cObj) {
        if ($.subKeyDefined(cObj, '_existingData.tags')) {
          ...
          $.socrataServer.makeRequest({
            type: 'GET',
            url: '/api/tags.json?method=viewsTags',
        ...
        var processResults = _.after(2, function() {
          if ($.isBlank(fedDomains)) {
            fedDomains = _.map(_.sortBy(_.select(rawFeds, function(f) {
              ...
              return {
                text: f.sourceDomainCName,
                value: f.sourceDomainId,
          ...
          callback([{
            text: 'This site only',
            value: domain.id,
            icon: '/api/domains/' + domain.cname + '/icons/smallIcon'
          }].concat(fedDomains));
        });
  ...
  var renderUpdate = function() {
    ...
    if (cObj._facet != newFacet) {
      ...
      cObj.$listSection.append($.tag2({
        _: 'li',
        contents: {
          _: 'a',
          href: '#clear',
          className: 'clearFacet',
      ...
      var itemHash = function(item) {
        return {
          ...
          contents: [{
            ...
            i: !$.isBlank(item.children),
            t: {
              _: 'ul',
              className: 'childList',
              contents: _.map(item.children, function(child) {
                return itemHash(child);
              })
            }
          }]
        };
      ...
      var renderItems = function(items) {
        ...
        if (cObj._facet.useCutoff) {
          if (!$.isBlank(catConfig)) {
            ...
            Configuration.findByType('catalog', null, function(config) {
              catConfig = config;
              setUpCutoff(cObj);
            });
          }
  ...
  var setUpCutoff = function(cObj) {
    ...
    if (cObj._facet.useModal) {
      ...
      links = _.sortBy(links, function(item) {
        return $(item).text();
      });
    ...
    cObj.$listSection.append($.tag2({
      ...
      contents: {
        ...
        href: '#viewAll',
        contents: $.t('controls.browse.actions.all_options')
      }
    }));
    cObj.$listSection.append($.tag2({
      _: 'li',
      ...
      contents: {
        _: 'a',
        className: ['viewLessLink', 'ss-directup'],
        href: '#viewLess',
  ...
  var filterClick = function(cObj, e) {
    ...
    if ($a.hasClass('cloudLink')) {
      ...
      _.defer(function() {
        cObj.$dialog.find('.optionsContent a:first').focus();
      });
      return;
    }
