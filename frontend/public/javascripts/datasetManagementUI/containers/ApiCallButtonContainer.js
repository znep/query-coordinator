...
export const findMatchingApiCall = (apiCalls, operation, callParams) =>
  _.find(apiCalls, call => {
    ...
    return _.isMatch(call, { operation, callParams }) && timeElapsed < SHOW_RESULT_STATE_FOR_MS;
  });

export const mapStateToProps = ({ ui }, ownProps) => {
  const apiCall = findMatchingApiCall(ui.apiCalls, ownProps.operation, ownProps.callParams);
