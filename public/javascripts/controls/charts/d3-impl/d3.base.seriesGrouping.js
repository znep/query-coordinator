  ...
  d3base.seriesGrouping = {
    ...
    initializeVisualization: function() {
      ...
      if (vizObj._fixedColumns[0]) {
        ...
        if (_.any(orderBys,
            function(ob) {
              return ob.columnFieldName == vizObj._fixedColumns[0].fieldName;
            })) {
      ...
      if (vizObj._fixedColumns[0]) {
        ...
        var categoryGroupedViewMetadata = $.extend(true, {}, sortedView.metadata);
        categoryGroupedViewMetadata.jsonQuery.group = categoryGroupBys;
        categoryGroupedView.update({
          metadata: categoryGroupedViewMetadata
        });
      }
      ...
      var seriesGroupedViewMetadata = $.extend(true, {}, sortedView.metadata);
      seriesGroupedViewMetadata.jsonQuery.group = seriesGroupBys;
      seriesGroupedView.update({
        metadata: seriesGroupedViewMetadata
      });
      ...
      var colorBasis = _.map(vizObj._valueColumns, function(col) {
        return col.color;
      });
    ...
    _preprocessSeriesColumns: function(continuation) {
      ...
      var sortedCategories = sg.categorySort == 'alphabetical' ? sg.categoryGroupedRows : _.sortBy(sg.categoryGroupedRows, 'id');
    ...
    renderData: function(data, view, didInsertData) {
      ...
      if (vizObj._inRenderSeriesGrouping) {
        return;
      }

      // If there were no insertions, don't bother re-rendering.
      if (didInsertData === false && vizObj._primaryView.totalRows() !== 0) {
        return;
      }
    ...
    _setLoadingOverlay: function() {
      if (!this._displayFormat.hideDsgMsg) {
        ...
        var overlay = $.tag({
          tagName: 'div',
          'class': 'dsgProgress flash notice invisible',
          contents: [{
            tagName: 'div',
            contents: [{
              tagName: 'span',
              'class': 'dsgOperationText'
            }]
          }, {
            tagName: 'p',
            contents: [{
              tagName: 'span',
              'class': 'dsgProgressText'
            }]
          }, {
            tagName: 'p',
            'class': 'dsgLoadingMsg',
            contents: [{
              tagName: 'span',
              'class': 'dsgPauseExplanationText',
              contents: $.t('controls.charts.series_grouping.pause_button_explanation1')
            }, {
              tagName: 'span',
              'class': 'filter dsgFilterIcon',
              contents: [{
                tagName: 'span',
                'class': 'icon'
              }]
              ...
              tagName: 'span',
              'class': 'dsgPauseExplanationText',
              contents: $.t('controls.charts.series_grouping.pause_button_explanation2')
            }]
          }, {
            tagName: 'p',
            contents: [{
              tagName: 'a',
              'class': 'button dsgProgressPauseButton invisible',
              contents: $.t('controls.charts.series_grouping.pause_rendering')
            }]
          }, {
            tagName: 'div',
            'class': 'loadingSpinner minimal dsgSpinner'
          }]
        }, true);
              ...
              if (!inPreprocess) {
                ...
                if (elapsedTimeMillisec > vizObj._remainingTimeDisplayDelayMillisec && sg.virtualRowReadyCount > 0) {
                  ...
                  if (seconds >= 60) {
                    ...
                    progressMessage = $.t('controls.charts.series_grouping.rendering_progress_seconds', {
                      rows_remaining: remaining,
                      time_remaining: seconds
                    });
                  } else {
                    // TODO converting to strings as $.t() thinks 0 is the same as undefined. Remove when appropriate (1 more instance below).
                    progressMessage = $.t('controls.charts.series_grouping.rendering_progress_almost_done', {
                      rows_remaining: remaining + ''
                    });
                  }

                } else {
                  progressMessage = $.t('controls.charts.series_grouping.rendering_progress', {
                    rows_remaining: remaining + ''
                  });
                }
              } else {
    ...
    _handleConditionalFormattingChanged: function() {
      ...
      this._seriesGrouping.sortedView.update({
        metadata: newMd
      });
    ...
    _getCacheKey: function(sg, valCols) {
      return '/' + (sg.fixedColumn || {
          fieldName: ':id'
        }).fieldName +
        '/' + _.map(valCols, function(c) {
          return c.fieldName;
        }).join(':') +
        '/' + _.map(sg.sortColumns, function(c) {
          return c.fieldName;
        }).join(':');
    }
  };
