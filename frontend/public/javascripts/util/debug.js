...
(function() {
  ...
  if (debug) {
    _.each(debug.split(','), function(key) {
      blist.debug[key] = true;
    });
  }
})();
...
window.mapDebugger = function() {
  ...
  window.viewConfig = window.mapObj._byView[window.mapObj._primaryView.id];

  window.zoomLevel = function() {
    console.log(window.mapObj.currentZoom());
  };
  window.zoomIn = function() {
    window.mapObj.map.zoomIn();
  };
  window.zoomOut = function() {
    window.mapObj.map.zoomOut();
  };
  window.hideFeature = function(feature) {
  ...
  window.animations = [];
  window.animationDebugger = function() {
    return {
      log: [],
      timestamps: function() {
        return _.pluck(this.log, 'timestamp');
      },
      get: function(index) {
        return this.log[index].timestamp;
      },
      totalMS: function() {
  ...
  window.newAnimationSet = function(animationConfig, num) {
    window.animations.push($.extend(window.animationDebugger(), {
      config: animationConfig,
      toAnimate: num,
      startAt: window.timestamp()
    }));
  };
  window.rememberAnimation = function(stuff) {
    _.last(window.animations).log.push($.extend({
      timestamp: window.timestamp()
    }, stuff));
  };
  window.killingAnimations = function() {
    _.last(window.animations).killedAt = window.timestamp();
  };
...
window.chartDebugger = function() {
  ...
  window.cc = window.chartObj._chartConfig || window.chartObj._columnChart;
  window.sg = window.chartObj._seriesGrouping;
