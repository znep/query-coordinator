(function() {
  ...
  var DatasetProxy = Backbone.Model.extend({
    initialize: function(_, options) {
      if (options && !$.isBlank(options.dataset)) {
  ...
  var Indicator = Backbone.Model.extend({
    ...
    initialize: function(_, options) {
      ...
      if (!this.get('dataset')) {
        this.set('dataset', new DatasetProxy());
      }

      var datasetProxy = this.get('dataset');
      this.set('date_column', new ColumnProxy({
        field_name: this.get('date_column')
      }, {
        datasetProxy: datasetProxy,
        acceptableTypes: 'calendar_date'
      }));
      this.set('column1', new ColumnProxy({
        field_name: this.get('column1')
      }, {
        datasetProxy: datasetProxy,
        acceptableTypes: numericTypes
      }));
      this.set('column2', new ColumnProxy({
        field_name: this.get('column2')
      }, {
        datasetProxy: datasetProxy,
        acceptableTypes: numericTypes
      }));
    ...
    toJSON: function() {
      ...
      _.each(_.keys(self.model).concat(['column1', 'column2', 'date_column']),
        function(k) {
          result[k] = self.attributes[k].toJSON();
        });
      result.compute_function = {
        column_function: result.column_function || 'null',
        aggregation_function: result.aggregation_function || 'sum',
        aggregation_function2: result.aggregation_function2 || '',
  ...
  var Metric = Backbone.Model.extend({
    ...
    initialize: function() {
      if (!this.get('current')) {
        this.set('current', new Indicator(null, {
          indicatorType: 'current'
        }));
      }
      if (!this.get('baseline')) {
        this.set('baseline', new Indicator(null, {
          indicatorType: 'baseline'
        }));
      }
    },
    isComplete: function() {
    ...
    toJSON: function() {
      var self = this;
      var result = Backbone.Model.prototype.toJSON.call(this);
      _.each(_.keys(self.model), function(k) {
        result[k] = self.attributes[k].toJSON();
      });
      result.comparison = {
        comparison_function: result.comparison_function || '<',
        time_to_compare: 'now'
      };
  ...
  var Goal = Backbone.Model.extend({
    ...
    initialize: function() {
      if (!this.get('agencies')) {
        this.set('agencies', new Agencies());
      }
      if (!this.get('related_datasets')) {
        this.set('related_datasets', new DatasetsProxy());
      }
      if (!this.get('metrics')) {
        this.set('metrics', new Metrics());
      }
    ...
    parse: function(response) {
      ...
      if ($.isBlank(response.metrics)) {
        response.metrics = new Metrics();
      }
      _.each(_.keys(response.metadata.metrics || {}).sort(), function(mI) {
        response.metrics.add(new Metric(response.metadata.metrics[mI], {
          parse: true
        }), {
          at: mI
        });
    ...
    toJSON: function() {
      ...
      _.each(_.keys(self.model), function(k) {
        attrs[k] = _.compact(self.attributes[k].toJSON());
      });
  ...
  var Goals = Backbone.Collection.extend({
    ...
    initialize: function(__, options) {
      ...
      this.on('add', function(goal) {
        ...
        this.listenTo(goal, 'removeFromAll', function() {
          this.remove(goal);
        });
      });
  ...
  Category.getColor = function(catList) {
    var usedColors = $.isBlank(catList) ? [] : catList.map(function(cat) {
      return cat.get('color');
    });
  ...
  var Categories = Backbone.Collection.extend({
    ...
    initialize: function() {
      ...
      this.on('add', function(category) {
        this.listenTo(category, 'removeFromAll', function() {
