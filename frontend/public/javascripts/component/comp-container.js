...
(function($) {
  $.component.Component.extend('Container', 'content', {
    ...
    _moveChildDom: function(child) {
      ...
      var cObj = this;
      child.bind('shown', function() {
        cObj._arrange();
      }, this);
      child.bind('hidden', function() {
        cObj._arrange();
      }, this);
    ...
    empty: function() {
      ...
      _.each(this._funcChildren, function(c) {
        c.destroy();
      });
    ...
    setVisibility: function() {
      ...
      this.each(function(c) {
        c.setVisibility.apply(c, origArgs);
      });
    },

    // Function signature: (start, count, callback)
        ...
        if (designing) {
          this.$dom.nativeDropTarget({
            ...
            dragOverCallback: function() {
              return cObj._handleDragOver.apply(cObj, arguments);
            },
      ...
      if (dropInfo.type == 'move') {
        ...
        if (!$.isBlank(moveChild) && moveChild.parent == this &&
          ...
          if (!$.isBlank(this._$dropCursor)) {
            this._$dropCursor.addClass('hide');
          }
    ...
    _getDropOrientation: function(child, pos) {
      ...
      return {
        vPos: mpY >= pos.y ? 'top' : 'bottom',
        hPos: mpX >= pos.x ? 'left' : 'right'
      };
    },
  ...
  $.component.Container.extend('Horizontal Container', 'content', {
    ...
    _arrange: function() {
      ...
      var totalWeight = 0;
      this.each(function(child) {
        if (child._isHidden || !child._isRenderable()) {
          return;
        }
