(function($) {
  $(function() {
    ...
    var goal = new govstatNS.models.Goal({
      id: $('.goalUid').text()
    });
    goal.fetch({
      success: function() {
    ...
    var chartHeight = function() {
      return $('.chartArea .meter').height();
    };
    var prepareChart = function(_goal, idx, $wrapper, color) {
      var $theContainer = $wrapper.children('.content-wrapper'); // todo: ew. clean up.

      // respect chart suppression
      if ((_goal.get('metadata') || {}).suppress_chart === true) {
        return false;
      }

      // get our data
      var metric = _goal.get('metrics').at(idx);
      if ($.isBlank(metric)) {
        return false;
      } // we don't have anything to show here

      var computedValues = metric.get('computed_values');
      if ($.isBlank(computedValues)) {
        return false;
      } // we still don't have anything to show

      var hasBaseline = _.isNumber(computedValues.baseline_value);

      var hasProjection = !$.isBlank(computedValues.projected_values);
      computedValues.projected_value;

      var dataSeries = computedValues.values;
      var projectionSeries = computedValues.projected_values || [];

      if ($.isBlank(dataSeries)) {
        return false;
      } // stilllll nothing interesting to show.
      if (dataSeries.length < 2) {
        return false;
      } // single-point paths don't visibly manifest.
      ...
      _.each([dataSeries, projectionSeries], function(series) {
        _.each(series, function(point) {
          point.x = timeParse(point.x);
        });
      });

      var startDate = timeParseZ(_goal.get('start_date'));
      var endDate = timeParseZ(_goal.get('end_date'));

      // scales
      var xScale = d3.time.scale().domain([startDate, endDate]);

      var allYPoints = dataSeries.concat(projectionSeries).concat([{
        y: computedValues.target_value
      }]);
      if (hasBaseline) {
        allYPoints.push({
          y: computedValues.baseline_value
        });
      }
      ...
      // d3/svg init
      var svg = d3.select($theContainer.get(0)).append('svg:svg');
      var container = d3.select($theContainer.get(0));
      ...
      if (color) {
        dataSeriesAreaPath.style('fill', color);
      }
      ...
      if (color) {
        dataSeriesLinePath.style('stroke', color);
      }
      ...
      if (metric.get('comparison_function') == '>') {
        $theContainer.find('.meter').
          find('.rangeTop').addClass('progress-good').end().
          find('.rangeBottom').addClass('progress-bad').end();
        computeProgress = function(val) {
          return val > computedValues.target_value ? 'good' : 'bad';
        };
      } else {
        $theContainer.find('.meter').
      ...
      var processXTicks = function(proposed, scale) {
        ...
        _.each(proposed, function(tick) {
          var lastTick = last(ticks);
          if (
            (tick.getMonth() !== lastTick.getMonth() || tick.getFullYear() !== lastTick.getFullYear()) &&
      ...
      var render = function() {
        ...
        // dataSeries buffers
        $theContainer.find('.dataSeriesLeftBuffer').
          css('width', xScale(dataSeries[0].x)).
          css('height', height - yScale(dataSeries[0].y));
        $theContainer.find('.dataSeriesRightBuffer').
        ...
        container.selectAll('.yTick').data(yTicks).enter().append('div').classed('yTick', true).
          style('bottom', function(d) {
            return yScaleInv(d) + 'px';
          }).append('div').classed('bubble', true).each(text(function(d) {
            return $.commaify(d);
          })).append('div').classed('tip', true);

        // x ticks
        var xTicks = processXTicks(xScale.ticks(width / 150), xScale);
        xTicks.push(timeParseZ(_goal.get('end_date'))); // always render end date
        var xTickDivs = container.selectAll('.xTick').data(xTicks);
        xTickDivs.enter().append('div').classed('xTick', true).classed('bubble', true).each(text(function(d) {
          return timeFormat(d);
        })).append('div').classed('tip', true);
        xTickDivs.style('left', function(d) {
          return xScale(d) + 'px';
        });
        ...
        if (hasProjection) {
          // projection marker
          $theContainer.find('.projectionMarker').show().
            addClass('progress-' + (computedValues.progress || 'none')).
            css('bottom', yScaleInv(last(projectionSeries).y));
        } else {
          $theContainer.find('.projectionMarker').hide();
        }

        // current marker
        $theContainer.find('.currentMarker').addClass('progress-' + computeProgress(last(dataSeries).y)).
          css('bottom', yScaleInv(last(dataSeries).y));

        // target marker
        $theContainer.find('.targetMarker').css('bottom', yScaleInv(computedValues.target_value));

        if (hasBaseline) {
          // baseline marker
          $theContainer.find('.baselineMarker').css('bottom', yScaleInv(computedValues.baseline_value));
        }

        // no tip, do you eat em?
        $theContainer.find('.nowTip').css('left', xScale(last(dataSeries).x));


        // post-render massage for ie8/r2d3
        $theContainer.find('.goalUid').prev().addClass('r2d3Chart');
      };
      ...
      // pulldown/up
      $theContainer.find('.pull').on('click', function() {
