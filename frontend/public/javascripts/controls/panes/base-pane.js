(function($) {
  ...
  var uniformEnabled = function() {
    return !$.browser.msie || $.browser.majorVersion > 7;
  };
  ...
  var mixpanelUserErrorTracking = function(validator) {
    if (blist.mixpanelLoaded) {
      var isClassicVisualizationError = _.any(
        ...
        function(el) {
          return el.name.match(/chart_create/);
        }
      );
      ...
      var notRequiredFieldErrors = _.any(
        validator.errorList,
        function(err) {
          return err.message != $.t('core.validation.required');
        }
      );
      ...
      if (isClassicVisualizationError && notRequiredFieldErrors) {
        ...
        var chartType = _.find(
          validator.currentElements,
          function(input) {
            return input.dataset.origname === 'displayFormat.chartType';
          }
  ...
  $.validator.addMethod('data-onlyIfInput', function(value, element) {
    if (this.optional(element)) {
      return true;
    }
  ...
  $.validator.addMethod('data-custom-4x4uid', function(value, element) {
    var viewUid = value;
  ...
  $.validator.addMethod('data-custom-fieldName', function(value) {
  ...
  $.validator.addMethod('data-requiredTypes', function(value, element, param) {
    ...
    $.t('screens.ds.grid_sidebar.base.validation.file_format', {
      formats: formats
    });
  });
  ...
  $.validator.addMethod('data-validateMin', function(value, element, param) {
    ...
    return !_.isNaN(parseInt(value)) ?
      $.t('screens.ds.grid_sidebar.base.validation.at_least', {
        value: $(element).attr('data-validateMin')
      }) :
  ...
  $.validator.addMethod('data-validateMax', function(value, element, param) {
    ...
    return !_.isNaN(parseInt(value)) ?
      $.t('screens.ds.grid_sidebar.base.validation.no_greater', {
        value: $(element).attr('data-validateMax')
      }) :
      $.t('screens.ds.grid_sidebar.base.validation.numeric');
  });
  ...
  */
  ...
  $.controlPane = {
    ...
    buttons: {
      create: {
        text: $.t('screens.ds.grid_sidebar.base.buttons.create'),
        value: true,
        isDefault: true,
        requiresLogin: true
      },
      update: {
        text: $.t('screens.ds.grid_sidebar.base.buttons.update'),
        value: true,
        isDefault: true,
        requiresLogin: true
      },
      apply: {
        text: $.t('screens.ds.grid_sidebar.base.buttons.apply'),
        value: true,
        isDefault: true
      },
      done: {
        text: $.t('screens.ds.grid_sidebar.base.buttons.done'),
        value: false,
        isDefault: true
      },
      cancel: {
        text: $.t('screens.ds.grid_sidebar.base.buttons.cancel'),
        value: false,
        isCancel: true
      }
    }
  ...
  $.Control.extend('controlPane', {
    _init: function() {
      ...
      if (cpObj.settings.noReset) {
        cpObj.$dom().addClass('noReset');
      }
    },
    ...
    $content: function() {
      if ($.isBlank(this._$content)) {
        this._$content = $.tag({
          ...
          'class': ['commonForm', {
            value: 'minimal',
            onlyIf: this.settings.minimalDisplay
          }]
        });
    ...
    render: function(data, isTempData, completeCallback) {
      ...
      $pane.find('.line.custom').each(function() {
        cleanLine(cpObj, $(this));
      });
      $pane.find('[data-customContent]').each(function() {
        cleanSection(cpObj, $(this));
      });
      $pane.empty();
      ...
      var rData = {
        title: cpObj.getTitle(),
        subtitle: cpObj.getSubtitle(),
        sections: cpObj._getSections(),
        paneId: cpObj.$dom().attr('id'),
        data: data || {}
      };
      ...
      var doRender = function() {
            ...
            'section<-sections': {
              '@class+': function(arg) {
                ...
                return _.compact([arg.item.type, arg.item.name,
                    ...
                    $.arrayify(arg.item.customClasses))).join(' ');
              },
        ...
        if (!cpObj._isReadOnly()) {
          $pane.find('.formSection.selectable .sectionSelect').bind('click', function() {
            var $c = $(this);
    ...
    reset: function(isSoft) {
      var cpObj = this;
      if (isSoft && cpObj.$dom().is('.noReset')) {
        return;
      }
    ...
    _isValid: function($input) {
      ...
      if ($input.hasClass('customWrapper')) {
        ...
        if (!$.isBlank(customValidator)) {
          customValidator = customValidator.validate;
        }
  ...
  var renderColumnSelectOptions = function(cpObj, columnsObj, columnIdField, curVal, args) {
    ...
    if (_.isEmpty(invalidOptions)) {
      invalidOptions.push({
        tagName: 'option',
        contents: $.t('screens.ds.grid_sidebar.base.column_select.none'),
        disabled: 'disabled'
      });
    }
  ...
  var commonAttrs = function(cpObj, item, context) {
    ...
    var result = {
      id: item.name + '_' + (item.uniqueId || _.uniqueId()),
      name: item.name,
      title: item.prompt,
      disabled: isDisabled,
      ...
      'class': ['inputItem', {
          value: 'required',
          onlyIf: item.required &&
            !context.inRepeater
        }, {
          value: 'textPrompt',
          onlyIf: !$.isBlank(item.prompt) &&
            _.include(['text', 'textarea'], item.type)
        },
        item.notequalto
      ].concat(item.extraClass),
      ...
      'data-notequalto': {
        value: '.' + (item.notequalto || '').
          split(' ').join(', .'),
        onlyIf: !$.isBlank(item.notequalto)
      },
      'data-validateMin': {
        value: item.validateMin,
        onlyIf: !$.isBlank(item.validateMin)
      },
      'data-validateMax': {
        value: item.validateMax,
  ...
  var checkRequiredData = function(cpObj, contextData, field) {
    return _.all(getRequiredNames(cpObj, contextData, field),
      function(n) {
        return !$.isBlank(getValue(contextData, n));
      });
  };
  ...
  renderLineItem.color = function(cpObj, contents, args, curValue) {
    ...
    wrapper.contents = [];
    wrapper.contents.push({
      tagName: 'a',
      href: '#Color',
      title: $.t('screens.ds.grid_sidebar.base.color_select.prompt'),
      name: args.item.name,
      'class': ['colorControl', {
        value: 'advanced',
        onlyIf: args.item.advanced
      }],
      contents: $.t('screens.ds.grid_sidebar.base.color_select.prompt'),
      style: {
  ...
  renderLineItem.custom = function(cpObj, contents, args, curValue, defValue) {
    ...
    _.last(contents).contents = $.extend(commonAttrs(cpObj, $.extend({}, args.item, {
      ...
      tagName: 'div',
      'data-customId': u,
      'data-linkedField': $.arrayify(args.item.linkedField).join(',')
    });
    _.last(contents).contents['data-defaultValue'] = $.htmlEscape(JSON.stringify(defValue || ''));
    cpObj._customCallbacks[u] = args.item.editorCallbacks;
  };

  renderLineItem.file = function(cpObj, contents, args) {
    _.last(contents).contents = {
      tagName: 'div',
  ...
  renderLineItem.group = function(cpObj, contents, args) {
  ...
  renderLineItem.note = function(cpObj, contents, args) {
  ...
  renderLineItem.radioGroup = function(cpObj, contents, args, curValue, defValue) {
    ...
    var items = _.map(args.item.options, function(opt, i) {
      ...
      if (args.item.sectionSelector) {
        subLine = renderLine(cpObj, {
          ...
          item: $.extend({}, opt, {
            type: 'static',
            isInput: true,
            extraClass: 'radioSectionSelector',
            name: args.item.origName,
            items: args.item.options,
            pos: i
          })
        });
      ...
      var subLineDisabled = _.all(subLine, function(subline) {
        return _.any($.makeArray(subline.contents), function(c) {
          return c.disabled;
        });
      });
      ...
      checkSubData = function(item) {
        ...
        return _.any(item, function(d) {
          return checkSubData(d);
        });
      };
      ...
      return {
        tagName: 'div',
        'class': ['radioLine', opt.type, (opt.lineClass || '')],
    ...
    contents.push({
      tagName: 'div',
      'class': 'radioBlock' + (args.item.extraClass === undefined ? '' : ' ' + args.item.extraClass),
      contents: items
    });
  };
  ...
  renderLineItem.repeater = function(cpObj, contents, args, curValue) {
    ...
    var removeButton = {
      tagName: 'a',
      href: '#remove',
      title: $.t('screens.ds.grid_sidebar.base.buttons.remove'),
      'class': 'removeLink delete',
      contents: {
        tagName: 'span',
        'class': 'icon'
      }
    ...
    if (!isRO) {
      var templateLine = renderLine(cpObj, {
        item: $.extend({}, args.item.field, {
          lineClass: 'repeaterAdded'
        }),
        context: $.extend({}, args.context, {
          repeaterIndex: 'templateId',
          noTag: true,
          inRepeater: true,
          data: null
        })
  ...
  renderLineItem.select = function(cpObj, contents, args, curValue, defValue) {
    ...
    var attributes = commonAttrs(cpObj, $.extend({}, args.item, {
      dataValue: curValue
    }), args.context);
  ...
  renderLineItem.slider = function(cpObj, contents, args, curValue, defValue) {
    ...
    wrapper.contents.push($.extend(commonAttrs(cpObj, $.extend({}, args.item, {
      ...
      tagName: 'input',
      type: 'text',
      value: (_.isNumber(curValue) ? curValue : defValue),
      'data-scale': scale,
      readonly: true
    }));
  ...
  renderLineItem['static'] = function(cpObj, contents, args) {
    var val = _.isFunction(args.item.value) ?
  ...
  var renderLine = function(cpObj, args) {
    ...
    args.item = $.extend({}, args.item, {
      origName: args.item.name,
    ...
    if (!args.context.inputOnly) {
      ...
      contents.push({
        tagName: 'label',
        'for': args.item.name + '_' + args.item.uniqueId,
        ...
        'class': [{
          ...
          value: 'requiredBlank',
          onlyIf: required && $.isBlank(args.item.text)
        }],
  ...
  var cleanSection = function(cpObj, $section) {
    ...
    if (!$.isBlank(cleaner)) {
      cleaner = cleaner.cleanupCallback;
    }
    if (!_.isFunction(cleaner)) {
      return;
    }
  ...
  var hookUpLinkedField = function(cpObj, $field) {
    ...
    var adjustField = function(curValue, force) {
      ...
      $linkedItems.each(function() {
        ...
        if ($this.hasClass('addValue')) {
          ...
          vals[$this.attr('data-origName')] = isCheckbox ? $this.filter(':checked').length > 0 : $(this).val();
        }
      });
      if (_.size(vals) == 1) {
        vals = _.values(vals)[0];
      ...
      if (!force && _.isEqual(curVals, vals)) {
        return;
      }
    ...
    var defAdjField = function() {
      $field.trigger('resetToDefault');
    };

    if ($linkedItems.hasClass('addValue')) {
      $parRepeater.delegate('.inputItem', 'change', defAdjField).
        delegate('.removeLink', 'click', function() {
          _.defer(defAdjField);
        });
    } else {
  ...
  var hookUpChangeHandler = function(cpObj, $field, handler) {
    var handlerProxy = function(event) {
      handler.call(cpObj, $field, event);
    };
  ...
  var hookUpFields = function(cpObj, $container) {
    //*** Text Prompts
    $container.find('.textPrompt').example(function() {
    ...
    $container.find('.colorControl').each(function() {
      ...
      if (!$picker.hasClass('advanced')) {
        ...
        $picker.ColorPicker({ //eslint-disable-line new-cap
          color: currentColor,
    ...
    $container.find('.fileChooser').each(function() {
      ...
      var ft = _.map(ftOrig, function(t) {
  ...
  var checkRepeaterMaxMin = function(cpObj, $repeater) {
    ...
    if (numLines < (_.isNumber(min) ? min : 1)) {
      _.defer(function() {
        addRepeaterLine(cpObj, $button);
      });
    }
