...
class PhidippidesTest < Test::Unit::TestCase
  ...
  def phidippides
    @phidippides ||= Object.send(:include, Phidippides).new
  end
  ...
  def test_phidippides_connection_details
    assert(phidippides.phidippides_address)
    assert(phidippides.phidippides_port)
  end

  def test_service_end_point
    assert_equal("http://#{phidippides.phidippides_address}:#{phidippides.phidippides_port}", phidippides.service_end_point)
  end
  ...
  def test_update_page_metadata
    ...
    result = phidippides.update_page_metadata('desk-chek', :data => page_metadata, :request_id => 'request_id')
    assert_equal(page_metadata, result[:body])
  end
  ...
  def test_update_dataset_metadata
    prepare_stubs(:body => dataset_metadata, :path => 'datasets/desk-chek', :verb => :put, :request_body => dataset_metadata)
    result = phidippides.update_dataset_metadata('desk-chek', :data => dataset_metadata, :request_id => 'request_id')
    assert_equal(dataset_metadata, result[:body])
  end
  ...
  def test_raises_error_when_zookeeper_cannot_find_phidippides
    ::ZookeeperDiscovery.stubs(:get_json).raises(ZK::Exceptions::BadArguments)
    assert_raises(::Phidippides::ConnectionError) do
      phidippides.fetch_page_metadata('four-four')
    end
  end
  ...
  def prepare_stubs(options)
    @mock_response = stub
    @mock_response.stubs(:code => options[:code] || '200', :body => options[:body].try(:to_json), :kind_of? => options.fetch(:kind_of, true))
    @mock_request = {}
    ...
    "Net::HTTP::#{options[:verb].capitalize}".constantize.expects(:new).with("#{phidippides.service_end_point}/#{options[:path]}").returns(@mock_request)
    @mock_http = stub
    @mock_http.expects(:request)
    Net::HTTP.expects(:start).with(phidippides.phidippides_address, phidippides.phidippides_port).yields(@mock_http).returns(@mock_response)
  end
