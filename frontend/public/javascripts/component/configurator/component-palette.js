(function($) {
  $.Control.extend('pane_componentPalette', {
    _init: function() {
      ...
      if ($.getTemplate('componentPaletteContainer').length < 1) {
        $('#js-appended-templates').append($.tag2({
          _: 'div',
          className: 'componentPaletteContainer',
          contents: {
            _: 'div',
            className: ['componentBlock', 'clearfix'],
            contents: [{
              _: 'span',
              className: 'title'
            }, {
  ...
  $.cf.edit.addComponentPalette = function(name, components, priority) {
    ...
    $.Control.extend(paneClassName, null, {
      name: paneName,
      components: components,
      groupName: name.capitalize()
    }, 'pane_componentPalette');
