(function($) {
  ...
  $.Control.registerMixin('highcharts', {
    ...
    renderRow: function(row) {
      ...
      if (!$.isBlank(chartObj._xCategories)) {
        ...
        if ($.isBlank((ri || {}).x)) {
          hasRI = false;
          ri = {
            x: chartObj._xCategories.length
          };
      ...
      if (isDateTime(chartObj) && $.isBlank(basePt.x)) {
        return true;
      }
      ...
      var renderPoint = function(series) {
        ...
        var finishRenderPoint = function() {
          ...
          if ((!chartObj._rowIndices.hasOwnProperty(row.id) ||
            ...
            _.any(series.data, function(datum) {
              return datum.x == basePt.x;
            })) {
    ...
    resizeHandle: function() {
      var chartObj = this;
      if ($.isBlank(chartObj.chart) || chartObj._isLoading) {
        return;
      }
  ...
  var getColor = function(chartObj, id, obj) {
    if ($.isBlank(chartObj._availableColors) || chartObj._recheckColors) {
      ...
      if ($.isBlank(chartObj._availableColors) ||
        ...
        chartObj._origColors = {
          colors: colors.slice(),
          dataId: chartObj._primaryView.id
        };
  ...
  var doChartRedraw = function(chartObj, callRowsRendered) {
    ...
    if (callRowsRendered) {
      chartObj._needsRowsRendered = true;
    }
    if (--chartObj._chartRedrawCount == 0) {
  ...
  var createSeries = function(chartObj, name, yCol, row) {
    var series = {
      ...
      seriesValues: {},
      yColumn: yCol,
      groupId: _.map(chartObj._seriesColumns,
        function(sc) {
          return renderCellText(row, sc.column);
        }).join('|') || 'default'
    };
    _.each(chartObj._seriesColumns, function(sc) {
  ...
  var useSeries = function(chartObj, series) {
    ...
    if (chartObj._chartType == 'donut') {
      ...
      _.each(chartObj._seriesCache, function(s, index) {
        var updatedOptions = {
          showInLegend: index == 0,
          dataLabels: {
            enabled: index == chartObj._seriesCache.length - 1
          },
          innerSize: Math.round(segment * (index + 1)) + '%',
          size: Math.round(segment * (index + 2)) + '%'
        };
        s = $.extend(s, updatedOptions);
        if ($.subKeyDefined(chartObj, 'chart.series.' + index)) {
          chartObj.chart.series[index].options = $.extend(chartObj.chart.series[index].options,
            updatedOptions);
        }
      });
    }
  ...
  var loadSeriesSums = function(chartObj) {
    if (!chartObj._columnsReady || $.isBlank(chartObj._totalRows)) {
      return;
    }
    ...
    var columnsDoneLoading = function() {
      ...
      if (!_.isUndefined(chartObj._pendingRows)) {
        _.each(chartObj._pendingRows, function(r) {
          chartObj.renderRow(r);
        });
        delete chartObj._pendingRows;
      }
    };
    ...
    if (chartObj._dataGrouping) {
      _.each(chartObj._seriesCache, function(series) {
        seriesSumsCount++;
        getSeriesSums(chartObj, series, function() {
          gotSeriesSums();
        });
      });
    } else {
  ...
  var getSeriesSums = function(chartObj, series, callback) {
    ...
    _.each(chartObj._seriesColumns, function(sc) {
      ...
      fc.children.push({
        operator: 'EQUALS',
        columnFieldName: sc.column.fieldName,
        value: v
      });
      hasFilters = true;
    });
    ...
    _.each(chartObj._yColumns, function(c) {
      customAggs[c.data.id] = ['sum'];
    });
    newDS.getAggregates(function() {
      var seriesRemaindersCache = chartObj._seriesRemainders[series.groupId] = {};
      _.each(chartObj._yColumns, function(c) {
        seriesRemaindersCache[c.data.id] = newDS.columnForID(c.data.id).aggregates.sum;
      });
      chartObj._seriesSums[series.groupId] = $.extend(true, {},
        chartObj._seriesRemainders[series.groupId]);
      ...
      _.each(chartObj._seriesRemaindersPending[series.groupId], function(srp) {
        srp();
      });
  ...
  var createChart = function(chartObj) {
    ...
    var _drawMarkers = function(invertAxis, zeroAtTop, format, markerStore, axis, otherAxis) {
      ...
      _.each(format, function(marker, index) {
        ...
        if (_.isString(lineAt)) {
          lineAt = parseFloat(lineAt.replace(/[^0-9\.\+\-]/, ''));
        }
        if (!_.isNumber(lineAt) || _.isNaN(lineAt)) {
          return;
        }
        ...
        if (isErrorBar) {
          if (invertAxis) {
            lowBar = [
              ['M', commands[0][1] - (size / 2) - 1, commands[0][2]],
              ['L', commands[1][1] + (size / 2) + 1, commands[0][2]]
            ];
            highBar = [
              ['M', commands[0][1] - (size / 2) - 1, commands[1][2]],
              ['L', commands[1][1] + (size / 2) + 1, commands[1][2]]
            ];
          } else {
            lowBar = [
              ['M', commands[0][1], commands[0][2] - (size / 2) - 1],
              ['L', commands[0][1], commands[1][2] + (size / 2) + 1]
            ];
            highBar = [
              ['M', commands[1][1], commands[0][2] - (size / 2) - 1],
        ...
        if (hasSVG) {
          ...
          addTip($(markerStore[index].handle.element), {
            top: 5
          });
        } else {
    ...
    var chartRedraw = function() {
      if (!$.subKeyDefined(chartObj, 'chart.series.0')) {
        return;
      }
    ...
    var chartConfig = {
      ...
      xAxis: {
        title: {
          text: $.isBlank(xTitle) ? null : xTitle,
          style: {
            backgroundColor: '#ffffff',
      ...
      yAxis: {
        endOnTick: false,
        tickPixelInterval: chartObj.$dom().height() / 8,
        title: {
    ...
    if (_.include(['donut'], chartObj._chartType)) {
      chartConfig.chart.spacingTop = 35;
    } // Fix data labels on donut charts
    ...
    if (seriesType == 'pie') {
      ...
      typeConfig.dataLabels = {
        formatter: function() {
          return clipFormatter.apply(this, [false, this.point.name + this.point.nameSuffix, labelLength]);
        },
      ...
      events: {
        ...
        click: function() {
          if ($.subKeyDefined(chartObj._primaryView, 'highlightTypes.select.' + this.row.id)) {
            ...
            chartObj.$dom().trigger('display_row', [{
              row: null
            }]);
          } else {
    ...
    if (chartObj._displayFormat.stacking && (chartObj._yColumns.length > 1 || chartObj._dataGrouping)) {
      chartConfig.plotOptions.series =
        $.extend(chartConfig.plotOptions.series, {
          stacking: 'normal'
        });
    ...
    var loadChart = function() {
      ...
      if (isDateTime(chartObj)) {
        createDateTimeOverview(chartObj);
      }
  ...
  var rowsRendered = function(chartObj) {
    ...
    if (chartObj._useRemainders && !_.isEmpty(chartObj._seriesRemainders)) {
      ...
      var conditionalFormat = _.detect(chartObj._primaryView.metadata.conditionalFormatting,
        function(cf) {
          return cf.condition === true;
        });
      if (conditionalFormat) {
        otherRow.color = conditionalFormat.color;
      }
  ...
  var xPoint = function(chartObj, row, ind) {
    ...
    if (chartObj._xColumn &&
      ...
      if (_.isNumber(pt.x)) {
        pt.x *= 1000;
      } else if (!$.isBlank(pt.x)) {
        pt.x = Date.parse(pt.x).valueOf();
      }
    } else if (!$.isBlank(ind)) {
      pt.x = ind;
    }
  ...
  var yPoint = function(chartObj, row, value, series, basePt) {
    ...
    if (isPieTypeChart &&
      ...
      if (chartObj._pointSize && chartObj._segments[chartObj._pointSize.id]) {
        pCol = chartObj._pointSize;
        point.label.size = pCol.name;
        for (var j = 0; j < chartObj._numSegments; j++) {
          if (parseFloat(row.data[pCol.lookup]) <= chartObj._segments[pCol.lookup][j]) {
            point.radius = 4 + (4 * j);
            point.states.select = $.extend(point.states.select, {
  ...
  var createDateTimeOverview = function(chartObj) {
    ...
    var config = {
      ...
      xAxis: {
        ...
        minPadding: 0.03,
        maxPadding: 0.03,
        dateTimeLabelFormats: chartObj.chart.options.xAxis.dateTimeLabelFormats
      },
      yAxis: {
        labels: {
          enabled: false
  ...
  var secondChartSelect = function(chartObj, event) {
    ...
    chartObj._primaryView.update({
      displayFormat: $.extend({}, chartObj._displayFormat, {
        detailBounds: {
          min: eAxis.min,
          max: eAxis.max
        }
  ...
  var addPoint = function(chartObj, point, series, isOther, pointIndex) {
    ...
    if (!$.isBlank(chartObj.chart)) {
      ...
      if ($.isBlank(p)) {
        ...
        if (point.selected && !p.selected) {
          p.select(true, true);
        } else if (!point.selected && p.selected) {
          p.select(false, true);
        }
    ...
    if ($.isBlank(ri)) {
      ...
      for (var i = newI + 1; i < series.data.length; i++) {
        ...
        if (!$.isBlank(datum)) {
          chartObj._rowIndices[datum.row.id][series.id] = i;
        }
      }
    } else {
