(function($) {
  ...
  var t = function(str, props) {
    return $.t('screens.admin.home.' + str, props);
  };
  ...
  $(function() {
    ...
    if (!_.isUndefined(homeNS.features)) {
      ...
      var pureFileDataUrl = function(context) {
        ...
        if (!$.isBlank(imageSha)) {
          return generateFileDataUrl(imageSha, context.item.viewId);
        }
        return '';
      };

      var getFeatureType = function(feature) {
        if (feature.item.display == 'thumbnail') {
          return 'thumbnail';
        }
        if (feature.item.display == 'custom') {
          return 'custom';
        }
        return 'text';
      };
      ...
      var featureDirective = {
        '.featureWrapper': {
          'feature<-': {
            ...
            '.featureContentCustomSection img@alt': function() {
              return '';
            },
            '.featureContentThumbnailSection img@src': generateThumbnailUrl,
            '.featureContentThumbnailSection img@alt': function() {
              return '';
            },
      ...
      $.live('.featureContentRadio', 'click', function() {
      ...
      $.live('.featureRemoveButton', 'click', function(event) {
        ...
        homeNS.features = _.reject(homeNS.features, function(feat) {
          return feat.viewId == viewId;
        });
      ...
      commonNS.selectedDataset = function(ds) {
        ...
        if (_.detect(homeNS.features, function(feat) {
          return feat.viewId == ds.id;
        })) {
      ...
      $('.saveFeaturesButton').click(function(event) {
        ...
        $('.featuresWorkspace .featureBox').each(function() {
          ...
          if ($this.hasClass('thumbnail')) {
            ...
            feature.display = {
              title: $this.find('.featureContentTextHeadline').val().clean(),
              description: $this.find('.featureContentTextSubtitle').val().clean()
            };
          }
        ...
        $.ajax({
          ...
          success: function() {
            ...
            setTimeout(function() {
              $button.removeClass('disabled').empty().html($.tag([
                t('save_now'), {
                  tagName: 'span',
                  'class': 'icon'
                }
