...
(function($) {
  ...
  var pullConfig = function() {
    ...
    page.update({
      content: $.component.root().properties(),
  ...
  $.extend($.cf, {
    initialize: function(opts) {
      ...
      var $cont = $.tag2({
        _: 'div',
        className: 'cfMainContainer'
      });
      $bodyElement.find('.siteInnerWrapper').append($cont);

      var $top = $.tag2({
        _: 'div',
        className: 'cfEditingBar',
        contents: [{
          _: 'div',
          className: 'editTitle',
          contents: [{
            _: 'span',
            contents: $.t('dataslate.edit.editing') + '&nbsp;'
          }, {
            _: 'span',
            className: 'pageName',
            contents: page.name
          ...
          contents: [{
            ...
            contents: {
              ...
              contents: [{
                ...
                contents: {
                  _: 'a',
                  href: '#preview',
                  className: ['preview', 'ss-icon'],
                  title: $.t('dataslate.edit.preview_page'),
                  contents: 'desktop'
                }
              }, {
                _: 'li',
                contents: {
                  _: 'a',
                  href: '#interactive',
                  className: ['interactive', 'ss-icon'],
                  title: $.t('dataslate.edit.view_editor'),
                  contents: 'layout'
                }
              }]
            }
            ...
            contents: {
              _: 'a',
              href: '#undo',
              className: ['undo', 'button', 'ss-replay'],
            ...
            contents: {
              _: 'a',
              href: '#redo',
              className: ['redo', 'button', 'ss-refresh'],
            ...
            contents: {
              _: 'a',
              href: '#permissions',
              className: ['permissions', 'button', 'ss-users'],
            ...
            contents: {
              _: 'a',
              href: '#settings',
              className: ['settings', 'button', 'ss-settings'],
      ...
      $.cf.edit.registerListener(function(undoable, redoable) {
        ...
        $top.find('.undo').toggleClass('disabled', !undoable).
          attr('title', undoable ? '' : $.t('dataslate.configurator.undo_title'));
        $top.toggleClass('can-redo', redoable);
        $top.find('.redo').toggleClass('disabled', !redoable).
          attr('title', redoable ? '' : $.t('dataslate.configurator.redo_title'));
      });

      $viewsCont = $.tag2({
        _: 'div',
        className: 'cfViewsContainer'
      });
      $cont.append($viewsCont);

      $previewCont = $.tag2({
        _: 'div',
        className: ['cfPreviewWrapper', 'cfViewTypeWrapper'],
        contents: {
          _: 'div',
          className: 'cfViewInnerWrapper'
        }
      });
      $viewsCont.append($previewCont);
      $previewCont.css('background-color', $bodyElement.css('background-color')).
        find('.cfViewInnerWrapper').append($bodyElement.find('.siteContentWrapper'));
      $previewCont.resizable({
        handles: 'e',
        maxWidth: $viewsCont.width() * 0.8,
        minWidth: $viewsCont.width() * 0.2,
        resize: function() {
      ...
      $editCont = $.tag2({
        _: 'div',
        className: ['cfEditingWrapper', 'cfViewTypeWrapper']
      });
      $viewsCont.append($editCont);
      $editCont.css('background-color', $bodyElement.css('background-color')).
        append($.tag2({
          _: 'div',
          id: 'edit_' + editContent.id,
          className: 'editRoot'
        }));

      $('.socrataModalWrapper').on('submit', function() {
      ...
      $('.socrataModalWrapper').on('click', '#configuratorPermissions .actions .save', function(e) {
        ...
        newPage.update({
          permission: $permissionsDialog.find(':radio:checked').val()
        });
        $.cf.edit.dirty = true;
      ...
      $('.socrataModalWrapper').on('click', '#configuratorSettings .actions .save', function(e) {
        ...
        newPage.update({
          name: $settingsDialog.find('[name=pageTitle]').value()
        });
        ...
        if (!isCopy) {
          newPage.update({
            path: $settingsDialog.find('[name=pageUrl]').value()
          });
      ...
      $body.find('.siteOuterWrapper').fullScreen({
        fullHeightSelector: '.notUsedNoConflict'
      });
    ...
    edit: function(edit) {
      ...
      if (designing != edit) {
        ...
        if (designing) {
          originalConfiguration = _.map($.component.root(true), function(rootComponent) {
            return {
    ...
    settings: function() {
      ...
      $settingsDialog.find('form').validate({
        errorElement: 'span',
        ignore: ':hidden',
