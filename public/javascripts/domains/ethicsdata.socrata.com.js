...
;(function($){$(function()
{
    ...
    $('.ethicsSearchSubmit').click(function(event)
    {
        ...
        $.bbq.pushState({ q: $searchField.val() });
    });
    ...
    $(window).bind('hashchange', function(event)
    {
        ...
        {
            // we have no search query; if the search is shown, hide it.
            $searchField.val('').trigger('blur');

            var $searchWidget = $('#ethicsSearchWidget');
            if ($searchWidget.length > 0)
            {
                $searchWidget.animate({ height: 0 }, function()
                {
                    $searchWidget.remove();
                    $('.browseSection')
                        .slideDown();
    ...
    _.defer(function()
    {
        // kick off a hashchange at start to catch pageload
