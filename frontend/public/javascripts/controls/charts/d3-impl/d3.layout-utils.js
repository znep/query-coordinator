...
d3ns.SlottedCircleLayout = function($) {
  ...
  constructor.prototype._moveSlotsDown = function(startSlotIndex) {
    ...
    if (startSlotIndex >= 0) {
      // LHS
      for (slotIndex = startSlotIndex; slotIndex < this.slotCount / 2; slotIndex++) {
  ...
  constructor.prototype._moveSlotsUp = function(startSlotIndex) {
    ...
    if (startSlotIndex >= 0) {
      ...
      if (firstEligibleSlot !== null && firstEligibleSlot >= 1) {
        ...
        for (slotIndex = firstSlotWeNeedToMoveUp; slotIndex >= 0; slotIndex--) {
          if (!this._slotOccupied(slotIndex)) {
            ...
            for (i = firstEmptySlot; i < firstSlotWeNeedToMoveUp; i++) {
              datumToMove = this._getSlotDataForIndex(i + 1);
              this._setSlotDataForIndex(undefined, i + 1);
              this._placeDatumIntoSlot(datumToMove, i);
            }
      ...
      for (slotIndex = canonicalStartIndex; slotIndex >= toggleCanonical(this.firstRHSSlotAllowed); slotIndex--) {
      ...
      if (firstEmptySlotAbove !== null) {
        ...
        for (i = firstEmptySlotAbove; i < canonicalStartIndex; i++) {
          datumToMove = this._getSlotDataForIndex(toggleCanonical(i + 1));
  ...
  constructor.prototype._debugPrintStatus = function(showEmpty) {
    ...
    var val;
    console.log('RHS:');
    for (i = 0; i < this.slotCount / 2; i++) {
      var realIdx = -i - 1;
      data = this._getSlotDataForIndex(realIdx);
      val = data ? data.data.getName() + ' (' + data.data.getValueText() + ')' : '[empty]';
      if (data || showEmpty) {
    ...
    console.log('LHS:');
    for (i = 0; i < this.slotCount / 2; i++) {
      data = this._getSlotDataForIndex(i);
      val = data ? data.data.getName() + ' (' + data.data.getValueText() + ')' : '[empty]';
      if (data || showEmpty) {
        console.log(i + ': ' + val);
      }
  ...
  constructor.prototype.debugVerifyLayout = function() {
    ...
    var i;
    // RHS
    for (i = 0; i < this.slotCount / 2 - 1; i++) {
      var realIdx = -i - 1;
      d = this._getSlotDataForIndex(realIdx);
      if (d) {
...
d3ns.fontMetrics = function($) {
  ...
  var fm = {
    getFontSpec: function(element) {
      ...
      spec.hash = _.map(spec, function(v, k) {
        return k + ':' + v;
      }).sort().join();
