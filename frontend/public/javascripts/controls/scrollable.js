...
(function($) {
  $.fn.scrollable = function(options) {
    ...
    return this.each(function() {
      ...
      $this.find(config.prevSelector).mousedown(function() {
        if (!stepPrev($this)) {
          return;
        }
        timerPtr = setTimeout(function() {
        ...
        $(document).bind('mouseup.scrollable.repeatScroll', function() {
      ...
      $this.find(config.nextSelector).mousedown(function() {
        if (!stepNext($this)) {
          return;
        }
        timerPtr = setTimeout(function() {
  ...
  function updateButtons($this) {
    ...
    if (prevHiddenItem($this).length > 0) {
      $this.find(config.prevSelector).parent().removeClass(config.disabledClass);
    } else {
      $this.find(config.prevSelector).parent().addClass(config.disabledClass);
    }

    if (nextHiddenItem($this).length > 0) {
      $this.find(config.nextSelector).parent().removeClass(config.disabledClass);
    } else {
      $this.find(config.nextSelector).parent().addClass(config.disabledClass);
    }
  }
