...
(function($) {
  ...
  $.fn.datasetGrid = function(options) {
    ...
    if (!datasetGrid) {
      datasetGrid = new $.datasetGridObject(options, this[0]); //eslint-disable-line new-cap
    }
    return datasetGrid;
  };
  ...
  $.extend($.datasetGridObject, {
    ...
    prototype: {
      init: function() {
        ...
        $datasetGrid.
          ...
          blistTable({
            ...
            rowMods: function(rows) {
              rowMods(datasetObj, rows);
            },
          ...
          blistModel().
          options({
            blankRow: datasetObj.settings.editEnabled,
            filterMinChars: 0
          });
        ...
        $.live('#' + $datasetGrid.attr('id') +
          ...
          function(e) {
            hookUpRowMenu(datasetObj, this, e);
          });
      ...
      drillDown: function(drillLink) {
        ...
        var filter;
        var columnJson = {
          columnFieldName: filterColumn.fieldName,
      ...
      rowHandleRenderer: function(html, index, renderIndex, row, col, context) {
        ...
        if (_.isEmpty(options) || row.type == 'blank') {
          return;
        }
  ...
  var hookUpRowMenu = function(datasetObj, curCell) {
    var $cell = $(curCell);
    if (!$cell.data('row-menu-applied')) {
      ...
      $menu.dropdownMenu({
        ...
        openCallback: function($menuElement) {
          rowMenuOpenCallback(datasetObj, $menuElement);
        },
        linkCallback: function(event) {
          rowMenuHandler(datasetObj, event);
        },
        pullToTop: true
      });
  ...
  var rowMenuOpenCallback = function() {
  ...
  var rowMenuHandler = function(datasetObj, event) {
    ...
    var action;
    var rowId;
    // If # isn't present, but ends with /\d+ or /row-.+ then it is a row URL
  ...
  var rowMods = function(datasetObj, renderedRows) {
    _.each(renderedRows, function(r) {
      var $row = $(r.row);
      if (!$row.is('.blist-tr-open')) {
        return true;
      }
      $row.find('.blist-tdh[colId]').
        each(function(i, tdh) {
          var $tdh = $(tdh);
          if ($tdh.hasClass('setup')) {
            return true;
          }
  ...
  var setupHeader = function(datasetObj, col, $col, tipsRef) {
    ...
    $col.columnMenu({
      ...
      selectedColumns: function() {
        return $(datasetObj.currentGrid).blistTableAccessor().getSelectedColumns();
      },
      view: datasetObj._view
    });


    var tip = $col.find('.menuLink').socrataTip({
      message: $.t('controls.grid.menu'),
      parent: 'body',
      shrinkToFit: true,
      trigger: 'hover',
      isSolo: false
    ...
    if (col.renderType.sortable && !nested) {
      $col.find('.blist-th-name').socrataTip({
        trigger: 'hover',
        message: $.t('controls.grid.click_to_sort'),
        parent: 'body',
        isSolo: true
      });
    }
  ...
  var headerMods = function(datasetObj, col) {
    if (!datasetObj.settings._colTips) {
      datasetObj.settings._colTips = {};
    }
  ...
  var columnNameEdit = function(datasetObj, event, origEvent) {
    ...
    $(document).bind('mousedown.columnNameEdit-' + $th.data('column').id,
        function(e) {
          columnEditDocMouse(datasetObj, e, $th);
        }).
  ...
  var columnEditDocMouse = function(datasetObj, event, $th) {
    ...
    if (($target.is('.name-wrapper :input') &&
      ...
      $target.closest('#jqmAlert').length > 0) {
      return;
    }
