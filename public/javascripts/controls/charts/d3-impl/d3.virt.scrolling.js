...
(function($) {
  ...
  $.Control.registerMixin('d3_virt_scrolling', {
    ...
    initializeVisualization: function() {
      ...
      cc.yAxis = {
        min: parseFloat($.deepGet(vizObj, '_displayFormat', 'yAxis', 'min')),
      ...
      cc.valueMarkerLimits = {
        min: d3.min(valueMarkers),
      ...
      // chart).
      vizObj.getColumns();
      if (vizObj.requiresSeriesGrouping() && $.isBlank(vizObj._seriesGroupingSentinel)) {
        // but don't do this if it's already been done
      ...
      $dom.empty().append($.tag({
        tagName: 'div',
        'class': 'mondrian barChart orientation' + $.capitalize(cc.orientation),
        contents: [{
            tagName: 'div',
            'class': 'chartOuterContainer',
            contents: [{
              tagName: 'div',
              'class': 'chartContainer',
              contents: [{
                  tagName: 'div',
                  'class': 'chartRenderArea',
                  contents: '&nbsp;'
                } // if no contents, browser doesn't bother to scroll
              ]
            }, {
              tagName: 'div',
              'class': 'tickContainer'
      ...
      // init our renderers
      cc.chartRaphael = new Raphael(cc.$chartContainer.get(0), 10, 10); //eslint-disable-line no-undef
      ...
      cc.throttledResize = _.throttle(_.debounce(function() {
        vizObj.resizeHandle();
      }, 500), 500); // TODO: this is more blunt than we need
      cc.$baselineContainer.draggable({
        ...
        stop: function() {
          ...
          vizObj._primaryView.update({
            displayFormat: $.extend(true, {},
              vizObj._primaryView.displayFormat, {
                valueLabelBuffer: cc.valueLabelBuffer
              })
          });
        }
      });
    ...
    getOrientation: function() {
      ...
      var paramOrientation = $.urlParam(window.location.href, 'orientation');
      var chartTypeOrientation = this._chartType.indexOf('column') > -1 ? 'right' : 'down';
      return paramOrientation || chartTypeOrientation;
    },
    ...
    _updateLabelPositioning: function() {
      ...
      if ($yLabelVert.exists()) {
        ...
        outerLeft = cc.$chartOuterContainer.position().left;
    ...
    _updateLegendStyle: function() {
      ...
      var legendPosition = vizObj.legendPosition();
    ...
    _drawAxisLabels: function() {
      ...
      if (!$.isBlank(vizObj._displayFormat.titleY)) {
        inLegend = cc.orientation == 'down' && vizObj.hasLegend() && vizObj.legendPosition() == 'top';

        clsname = cc.orientation == 'right' ? 'yLabelVert' : 'xLabelHoriz';
      ...
      if (!$.isBlank(vizObj._displayFormat.titleX)) {
        clsname = cc.orientation == 'right' ? 'xLabelHoriz' : 'yLabelVert';
        cc.$chartArea.addClass('has' + $.capitalize(clsname));

        inLegend = cc.orientation == 'right' && vizObj.hasLegend() && vizObj.legendPosition() == 'bottom';
    ...
    _resizeEverything: function() {
      ...
      var chartArea = cc.$chartOuterContainer[cc.dataDim.width]();
      var domArea = cc.$chartOuterContainer[cc.dataDim.height]();
      var maxRenderWidth = vizObj._maxRenderWidth();
      var barWidthBounds = defaults.barWidthBounds;
      var barSpacingBounds = defaults.barSpacingBounds;
      var rowSpacingBounds = defaults.rowSpacingBounds;
      var sidePaddingBounds = defaults.sidePaddingBounds;
      ...
      if (minTotalWidth > chartArea) {
        ...
        if (maxTotalWidth < chartArea) {
          ...
          var scale = function(bounds) {
            return ((bounds[1] - bounds[0]) * scalingFactor) + bounds[0];
          };
    ...
    _recalculateDataOffset: function() {
      ...
      if (index >= 0 && cc.drawElementPosition > 0) {
        cc.dataOffset = vizObj._xDatumPosition(0)({
            index: index
          }) -
    ...
    _repositionDrawElement: function() {
      ...
      if ((scrollPosition < drawElementPosition) ||
        ...
        cc.drawElementPosition = $.clamp(scrollPosition - Math.floor(drawElementWidth / 2), [0, Math.ceil(cc.dataDim.pluckX(cc.chartWidth, cc.chartHeight) - drawElementWidth)]);
    ...
    _currentYScale: function() {
      ...
      if (usedMin === usedMax) {
        if (_.isNaN(explicitMin)) {
          usedMin -= nullRangePaddingAmount;
        }
        if (_.isNaN(explicitMax)) {
          usedMax += nullRangePaddingAmount;
        }
      }

      // Freaking out entirely, recognizing that the world is ending.
      if (!_.isNumber(usedMin) || _.isNaN(usedMin)) {
        usedMin = -nullRangePaddingAmount;
      }
      if (!_.isNumber(usedMax) || _.isNaN(usedMax)) {
        usedMax = nullRangePaddingAmount;
      }
    ...
    _renderTicks: function(oldYScale, newYScale, isAnim) {
      ...
      if ($.subKeyDefined(vizObj, '_displayFormat.yAxis.noDecimals')) {
        $.extend(formatter, {
          noDecimals: $.deepGet(vizObj, '_displayFormat', 'yAxis', 'noDecimals')
        });
      }
      ...
      var newLinePos = function(d) {
        return (yAxisPos + cc.dataDim.dir * newYScale(d)) + 'px';
      };
      ...
      if (allowTransitions) {
        ...
        tickLines.
          style(position, function(d) {
            return (extraOffset + yAxisPos + cc.dataDim.dir * oldYScale(d)) + 'px';
          }).
          transition().
      ...
      return _.compact(_.map(this._displayFormat.valueMarker, function(marker) {
        if (_.isNumber(marker.atValue)) {
          ...
          return $.extend(marker, {
            atValue: $.numericalSanitize(marker.atValue)
          });
        } else {
          return null;
        }
    ...
    _renderValueMarkers: function(valueMarkers, oldYScale, newYScale, isAnim) {
      ...
      var valueMarkerPosition = cc.dataDim.pluckY(
        function(yScale, extraOffsetY) {
          return function(d) {
            return (extraOffsetY + yAxisPos + yScale(parseFloat(d.atValue))) + 'px';
          };
        },
        function(yScale, extraOffsetX) {
          return function(d) {
            return (extraOffsetX + yAxisPos - yScale(parseFloat(d.atValue))) + 'px';
          };
        });

      var valueMarkersSelection = cc.chromeD3.selectAll('.valueMarkerContainer').
        data(valueMarkers);

      valueMarkersSelection.
        enter().append('div').
    ...
    _formatYAxisTicks: function(formatter) {
      ...
      if (formatter.abbreviate === true) {
        ...
        return function(num) {
          return $.commaify(num.toFixed(formatter.decimalPlaces));
        };
      } else {
        return $.commaify;
      }
    ...
    _renderRowLabels: function(data) {
      ...
      var rowLabels = cc.chartD3.selectAll('.rowLabel').
        data(_.filter(data, labelTransform.isInView), function(row) {
          return row.id;
        });
      ...
      rowLabels.
        ...
        text(function(d) {
          ...
          if ($.isBlank(fixedColumn)) {
            text = d.index + '';
          } else {
            // render plaintext representation of the data
            text = fixedColumn.renderType.renderer(d.data[fixedColumn.lookup], fixedColumn, true, null, null, true);
          }
    ...
    _labelTransform: function(offset) {
      ...
      offset = offset || cc.dataDim.pluckY(
        function(d) {
          return [yPositionStaticParts - 10, xPositionStaticParts + (d.index * cc.rowWidth)];
        },
        function(d) {
          return [xPositionStaticParts + (d.index * cc.rowWidth) - 3.5, yPositionStaticParts + 10];
        });
    ...
    _errorBarPath: function(yScale) {
      ...
      return function(d) {
        ...
        if (cc.orientation == 'right') {
          x = 0;
          y = yAxisPos - yScale(high);
          height = yScale(high) - yScale(low);
    ...
    _flyoutConfigurationOptions: function(row, col) {
      ...
      var lessThanZeroPositioning =
        cc.dataDim.pluckY(['top-left', 'bottom-left'], ['bottom', 'top']);

      var greaterThanZeroPositioning =
        cc.dataDim.pluckY(['top-right', 'bottom-right'], ['top', 'bottom']);
      return {
