...
function HistogramVisualizationService(
  ...
  function setupBrush(dom, scale) {
    ...
    brush.selectionClearFlyout = _.constant(
      `<div class="flyout-title">${I18n.distributionChart.dragClearHelp}</div>`
    );
    brush.brushDragFlyout = _.constant(`<div class="flyout-title">${I18n.distributionChart.dragHelp}</div>`
    );
  ...
  function updateBrush(
    ...
    function setupBrushHandles(selection, height, leftOffset) {
      ...
      function buildBrushHandle(side, path) {
        ...
        if (selection.select(`.histogram-brush-${side}`).empty()) {
          selection.select(`.resize.${direction}`).
            attr('transform', `translate(${Constants.HISTOGRAM_DRAG_TARGET_WIDTH + leftOffset}, 0)`).
            append('g').attr('class', `histogram-brush-${side}`).
            call(brushLine).
    ...
    brushClear.
      ...
      attr('transform', function(d) {
        return `translate(${d.brushLeft}, ${d.top + dom.margin.top})`;
      }).
      attr('height', '2em').
    ...
    brushClearTarget.
      attr('transform', function(d) {
        ...
        return `translate(${brushClearTextOffset + brushClearTextWidth - width}, 0)`;
      });
    ...
    dom.brush.selectAll('.background').
      ...
      attr('transform', `translate(${-dom.margin.left}, 0)`).
