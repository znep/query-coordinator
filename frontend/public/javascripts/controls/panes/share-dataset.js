(function($) {
  ...
  var commonError = function(cpObj) {
    cpObj.$dom().find('.sharingFlash').addClass('error').text($.t('screens.ds.grid_sidebar.share.error'));
  };
  ...
  var grabShares = function(cpObj, $context, grants) {
    ...
    ServerModel.sendBatch(function() {
      ...
      _.each(shares, function(share) {
        ...
        $li.find('select.type').val(shareType).end().find('span.type').text(shareType);
      ...
      $context.find('.type').change(function(event) {
        ...
        cpObj.settings.view.replaceGrant(
          $.extend({}, existingGrant, {
            type: $line.attr('data-currtype')
          }), existingGrant,
          function() {
  ...
  var updateShareText = function(cpObj, $context) {
    ...
    cpObj._view.userGrants(function(friends) {
      if (friends.length == 0) {
        $span.addClass('hide');
        $context.find('.noShares').show().end().find('.shareNotifyLink').hide();
      } else {
  ...
  $.Control.extend('pane_shareDataset', {
    _init: function() {
      ...
      cpObj._view.bind('permissions_changed', function() {
        cpObj.reset();
      }, cpObj);
    },
    ...
    getSubtitle: function() {
      return $.t('screens.ds.grid_sidebar.share.subtitle', {
        ...
        customContent: {
          ...
          callback: function($formElem) {
            ...
            $formElem.find('.shareNotifyLink').click(function(event) {
              event.preventDefault();
              cpObj._view.notifyUsers(function() {
  ...
  if (blist.sidebarHidden &&
    ($.isBlank(blist.sidebarHidden.manage) || !blist.sidebarHidden.manage.sharing)) {
