...
describe RequestHost do
  let(:environment_hash_without_x_socrata_host) {
   {
   "REQUEST_METHOD"=>"GET",
  ...
  let(:environment_hash_with_x_socrata_host) {
    environment_hash_without_x_socrata_host.merge("HTTP_X_SOCRATA_HOST" => "domain.in.x.socrata.host.header.com")
  }
  ...
  describe 'the environment hash' do
    ...
    context 'when the X-Socrata-Host header is set' do
      it 'replaces the existing SERVER_NAME with the contents of the header' do
        expect(app).to receive(:call).with(hash_including('SERVER_NAME' => 'domain.in.x.socrata.host.header.com'))
      ...
      it 'replaces the existing HTTP_HOST with the contents of the header' do
        expect(app).to receive(:call).with(hash_including('HTTP_HOST' => 'domain.in.x.socrata.host.header.com'))
        subject.call(environment_hash_with_x_socrata_host.deep_dup)
      end

      context 'with a valid REQUEST_URI' do
        it 'rewrites the existing REQUEST_URI use X-Socrata-Host as the domain' do
          expect(app).to receive(:call).with(hash_including('REQUEST_URI' => 'http://domain.in.x.socrata.host.header.com/some_path?query=value'))
          subject.call(environment_hash_with_x_socrata_host.deep_dup)
        end
      end
      ...
      context 'with an invalid REQUEST_URI' do
        ...
        it 'raises URI::InvalidURIError' do
          ...
          expect { subject.call(environment_hash_with_bad_request_uri) }.to raise_error(URI::InvalidURIError)
        end
      end
    end
    ...
    context 'when the X-Socrata-Host header is not set' do
      it 'keeps the existing SERVER_NAME' do
        expect(app).to receive(:call).with(hash_including('SERVER_NAME' => 'domain.in.host.header.com'))
        subject.call(environment_hash_without_x_socrata_host.deep_dup)
      end

      it 'keeps the existing HTTP_HOST' do
        expect(app).to receive(:call).with(hash_including('HTTP_HOST' => 'domain.in.host.header.com'))
        subject.call(environment_hash_without_x_socrata_host.deep_dup)
      end

      it 'keeps the existing REQUEST_URI' do
        expect(app).to receive(:call).with(hash_including('REQUEST_URI' => 'http://domain.in.host.header.com/some_path?query=value'))
        subject.call(environment_hash_without_x_socrata_host.deep_dup)
      end
    end
  end
