    ...
    prototype: {
      ...
      ready: function() {
        ...
        if (!$.isBlank(blob) && _.include(GOOGLE_VIEWER_TYPES, blob.type)) {
          var embedHtml,
            gviewerUrl = '//docs.google.com/gview?url=' + ds._generateBaseUrl() + blob.href;
          if (blob.type.indexOf('application/pdf') !== -1 && $.browser.msie) {
            embedHtml = '<div class="externalLink flash notice">Trouble viewing the document? Try using the <a href="' + gviewerUrl + '" target="_blank">Google Document Viewer</a>.</div>';
          ...
          $domObj.append($.tag2({
            _: 'div',
            className: ['displayArea', 'image', 'fullHeight'],
            contents: {
              _: 'img',
              alt: $.htmlEscape(ds.name),
              src: blob.href
            }
          }));
        } else {
      ...
      reload: function() {
        ...
        $.ajax({
          url: '/blob/' + blobObj.settings.view.id,
