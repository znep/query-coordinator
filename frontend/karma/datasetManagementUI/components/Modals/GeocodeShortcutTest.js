...
describe('components/Modals/GeocodeShortcut', () => {
  ...
  it('sets the mapping correctly from the AST of geocode/4', () => {
        ...
        "args": [
          {
            ...
            "args": [
              ...
              {"type":"funcall","function_name":"to_number","args":[{"value":"district","type":"column_ref"}]},
              {"type":"funcall","function_name":"to_number","args":[{"value":"beat","type":"column_ref"}]},
              {"type":"funcall","function_name":"to_number","args":[{"value":"ward","type":"column_ref"}]}
            ]
          }
        ]
      },
    ...
    const withGeocode = {
      ...
      output_schema_columns: {...entities.output_schema_columns, '137-99999': outputSchemaColumn},
    ...
    assert.deepEqual(
      component.instance().state.mappings
      .filter(([name, _]) => names.indexOf(name) >= 0)
      .map(([name, { id }]) => [name, id]),
      [
        ['address', 1930],
        ['city',    1932],
  ...
  it('hides the original columns when the user asks', () => {
    ...
    const newOutputColumnNames = component.instance().genDesiredColumns().map(oc => oc.field_name);
