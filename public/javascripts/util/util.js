...
String.prototype.endsWith = function(str) { //eslint-disable-line no-extend-native
...
String.prototype.visualSize = function(fontSize) { //eslint-disable-line no-extend-native
  var $ruler = $('#ruler');
  if ($ruler.length < 1) {
...
String.prototype.visualHeight = function(fontSize) { //eslint-disable-line no-extend-native
  return this.visualSize(fontSize).height;
};

String.prototype.visualLength = function(fontSize) { //eslint-disable-line no-extend-native
...
String.prototype.displayable = function() { //eslint-disable-line no-extend-native
...
String.prototype.clean = function() { //eslint-disable-line no-extend-native
  // Sometimes strings have &nbsp;, so replace them all with normal spaces
  return this.replace(/\xa0/g, ' ');
};

String.prototype.camelize = function() { //eslint-disable-line no-extend-native
  return _.map(this.split(/[\W_]/), function(item) {
    return item.substring(0, 1).toUpperCase() + item.substring(1);
  }).join('');
};

String.prototype.plainTextToHtml = function() { //eslint-disable-line no-extend-native
...
String.prototype.levenshtein = function(other) { //eslint-disable-line no-extend-native
  var l1 = this.length,
    l2 = other.length;
  if (Math.min(l1, l2) === 0) {
...
String.prototype.heuristicDistance = function(other) { //eslint-disable-line no-extend-native
  if ($.isBlank(other)) {
    return NaN;
  }
...
Function.prototype.curry = function() { //eslint-disable-line no-extend-native
  var f = this;
...
String.prototype.linkify = function(extra) { //eslint-disable-line no-extend-native
  if ($.isBlank(this)) {
    return '';
  }
  if ($.isBlank(extra)) {
...
(function($) {
  ...
  $.flattenChildren = function(array, key) {
    ...
    var recurse = function(arrayInner) {
  ...
  $.parseParams = function(query) {
    ...
    while (e = re.exec(query)) { //eslint-disable-line no-cond-assign
      var k = decode(e[1]);
      var v = decode(e[2]);
  ...
  $.serialPromiser = function(ary) {
    var result = $.Deferred(), //eslint-disable-line new-cap
  ...
  blist.util.doAuthedAction = function(actionText, callback) {
    if (!$.isBlank(blist.util.inlineLogin)) {
      blist.util.inlineLogin.verifyUser(
        ...
        }, $.t('controls.common.auth_required', {
          action_phrase: actionText
        }));
  ...
  blist.util.enforceLodashFunctions = function() {
    if (window._ !== window.socrataLodash) {
      if (window.console && typeof(window.console.info) === 'function') {
        console.info(
          'Lodash collision detected. Expected version: {0}. Actual version: {1}.'.format(window.socrataLodash.VERSION, window._.VERSION)
        );
      }
