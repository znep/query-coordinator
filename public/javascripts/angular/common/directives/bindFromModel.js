...
// <div bind-from-model="myModel.name, myModel.description">
//   <h1>{{name}}</h1>
//   <p>{{description}}</p>
// </div>
angular.module('socrataCommon.directives').directive('bindFromModel', function(AngularRxExtensions) {
  return {
    restrict: 'A',
    scope: true,
    link: function(scope, element, attrs) {
      ...
      var properties = attrs['bindFromModel'].split(',');
      ...
      _.each(properties, function(keyPathString) {
        ...
        if (keyPath.length < 2) {
          ...
          throw new Error('bindFromModel expects two or more entries in the key path, the first being the model name on the scope and the second a property name on the model.');
        }
