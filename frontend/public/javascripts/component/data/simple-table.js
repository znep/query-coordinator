    ...
    var setUpTable = function() {
        ...
        if (_.isArray(cObj._properties.columns)) {
            ...
            cObj._columns = _.chain(cObj._dataContext.dataset.visibleColumns).filter(function(col) {
                ...
                return { id: col.fieldName, text: col.name };
            }).value();
        }
        ...
        if ($.isBlank(cObj._dataContext)) {
            ...
            view.getRows((cObj._stringSubstitute(cObj._properties.rowBodyPage || 1) - 1) * rowCount, rowCount,
                function(rows) {
                    cObj.$tbody.empty();
                    callback = doneWithRowsCallback(rows.length);
            ...
            cObj.$tbody.empty();
            callback = doneWithRowsCallback(cObj._dataContext.datasetList.length);
...
var addRow = function(cObj, $parent, config, row, rowIndex, rowLength, callback) {
    ...
    var addCell = function(cell, configuration, colIndex, colLength, colId) {
        ...
        $row.append($.tag({ tagName: configuration.isHeader || cell.isHeader ? 'th' : 'td',
            'class': _.compact([colId, cell.htmlClass]),
            scope: { onlyIf: configuration.isHeader && cell.isColumn || cell.isHeader,
                value: configuration.isHeader && cell.isColumn ? 'col' : 'row' },
            contents: cell.value }));
    };
