...
(function(root) {
  ...
  if (!_.has(root, 'socrata.visualizations.ChoroplethMapUtils')) {
    throw new Error(
      '`{0}` must be loaded before `{1}`'.
        format(
  ...
  if (!_.has(root, 'd3')) {
    throw new Error('d3 is a required dependency for `socrata.visualizations.ChoroplethMap.js`.');
  }

  if (!_.has(root, 'ss')) {
    throw new Error('ss (simple-statistics) is a required dependency for `socrata.visualizations.ChoroplethMap.js`.');
  }

  if (!_.has(root, 'chroma')) {
    throw new Error('chroma is a required dependency for `socrata.visualizations.ChoroplethMap.js`.');
  }
  ...
  function ChoroplethMap(element, vif) {
    ...
    var _visualizationUtils = new root.socrata.visualizations.ChoroplethMapUtils(_utilConstants);
  ...
  root.socrata.visualizations.ChoroplethMap = ChoroplethMap;
})(window);
