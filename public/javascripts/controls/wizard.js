...
(function($) {
    $.fn.wizard = function(options) {
        ...
        return this.each(function() {
            ...
            // append control row
            var $wizardButtons = $.tag({
                tagName: 'ul',
                'class': 'wizardButtons clearfix',
                'aria-live': 'polite',
                contents: [{
                    tagName: 'li',
                    'class': 'cancel',
            ...
            var animateHoriz = function() {
                $paneContainer.stop().animate({
                    marginLeft: -1 * $currentPane.position().left
                }, function() {
            ...
            var prevPane = function(prevAttr) {
                ...
                // grab new currents
                var $paneToShow = _.last(paneStack);
                var prevState = _.last(stateStack);

                // go there
                $currentPane.before($paneToShow);
                $paneToShow.show();
                $paneContainer.css('marginLeft', -1 * $paneToShow.outerWidth());
                activatePane($paneToShow, prevState);
                animateHoriz();
            };
