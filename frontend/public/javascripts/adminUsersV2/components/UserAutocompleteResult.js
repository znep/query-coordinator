...
export class UserAutocompleteResult extends Result {
  ...
  displayTitle() {
    ...
    // screen_name match
    const screenNameMatch = matches.find((match) => { return match.field === 'screen_name'; });
    if (!_.isUndefined(screenNameMatch)) {
      ...
      screenNameMatch.offsets.forEach((offset, index) => {
        ...
        displayTitleFragments.push(
          screen_name.substring(offset.length + offset.start, nextMatchStart)
        );
      });
    } else {
    ...
    // email match
    const emailMatch = matches.find((match) => { return match.field === 'email'; });
    if (!_.isUndefined(emailMatch)) {
      ...
      emailMatch.offsets.forEach((offset, index) => {
        ...
        const nextMatchStart = nextMatch ? nextMatch.start : undefined;
        displayTitleFragments.push(
          email.substring(offset.length + offset.start, nextMatchStart)
        );
      });
    } else {
...
const ConnectedUserAutocompleteResult = connect(
  null,
  mapDispatchToProps
