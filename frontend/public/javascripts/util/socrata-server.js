(function() {
    ...
    makeRequest: function(req) {
      ...
      var finishCallback = function(isSuccess, callback, allCompleteCallback) {
        return function(dataset, statusMsg, xhr) {
          ...
          if (statusCode == 202) {
            ...
            if (_.isFunction(req.pending)) {
              req.pending.apply(this, arguments);
            }
            ...
            setTimeout(function() {
              isCache ? $.Tache.Get(req) : $.ajax(req); //eslint-disable-line new-cap
            }, retryTime);
            return;
      ...
      if (!req.isSODA) {
        if (!$.isBlank(model.accessType)) {
          ...
          $.debug('making call without accessType!', req);
        }
      }
      ...
      if (req.pageCache) {
        cleanReq();
        $.Tache.Get(req); //eslint-disable-line new-cap
  ...
  ServerModel.sendBatch = function(successCallback, errorCallback, completeCallback) {
    ...
    if (batchRequests.length == 1) {
      ...
      $.ajax($.extend({}, origBR, {
        complete: function() {
          if (_.isFunction(origBR.complete)) {
            origBR.complete();
          }
        ...
        error: function(xhr) {
          ...
          if (_.isFunction(origBR.error)) {
            origBR.error(errBody.message);
          }
          if (_.isFunction(errorCallback)) {
            errorCallback(errBody.message);
          }
        },
        success: function(resp) {
          if (_.isFunction(origBR.success)) {
            origBR.success(resp);
          }
          if (_.isFunction(successCallback)) {
            successCallback();
